 ...
 * Each event you receive from the library will have its parameter derived from
 * this class. The class contains the raw event data, and a pointer to the
 * current shard's dpp::discord_client object. You can also use this object to
 * cancel the current event, meaning that any listeners after yours do not get
 * notified of the current event if you call it.
 */
struct DPP_EXPORT event_dispatch_t {
  /**
   * @brief Raw event data.
   * If you are using json on your websocket, this will contain json, and if you
   * are using ETF as your websocket protocol, it will contain raw ETF data.
   */
   ...
   *
   * @param client The shard the event originated on. May be a nullptr, e.g. for
   * voice events
  ...
  /**
   * @brief Cancels the event in progress. Any other attached lambdas for this
   * event after this one are not called. Note that event cancellation is a
   * thread local state, and not stored in the object (because object which can
   * be cancelled is `const` during the event, and cannot itself contain the
   * changeable state).
   * @return const event_dispatch_t& reference to self for chaining
   */
  [[nodiscard]] const event_dispatch_t &cancel_event() const;

  /**
   * @brief Returns true if the event is cancelled.
   * Note that event cancellation is a thread local state, and not stored in the
   * object (because object which can be cancelled is `const` during the event,
   * and cannot itself contain the changeable state).
 ...
 */
std::function<void(const dpp::confirmation_callback_t &detail)>
	DPP_EXPORT log_error();
}; // namespace utility
...
struct DPP_EXPORT guild_scheduled_event_user_add_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_scheduled_event_user_add_t(class discord_client *client,
...
struct DPP_EXPORT guild_scheduled_event_user_remove_t
	: public event_dispatch_t {
  /** Constructor
   * @param client The shard the event originated on. CAN BE NULL
   * for log events originating from the cluster object
   * @param raw Raw event text as JSON
   */
  guild_scheduled_event_user_remove_t(class discord_client *client,
                                      const std::string &raw);
  /**
...
struct DPP_EXPORT guild_scheduled_event_create_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_scheduled_event_create_t(class discord_client *client,
                                 const std::string &raw);
  /**
   * @brief created event
   */
...
struct DPP_EXPORT guild_scheduled_event_update_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_scheduled_event_update_t(class discord_client *client,
                                 const std::string &raw);
  /**
   * @brief updated event
   */
...
struct DPP_EXPORT guild_scheduled_event_delete_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_scheduled_event_delete_t(class discord_client *client,
                                 const std::string &raw);
  /**
   * @brief deleted event
   */
...
struct DPP_EXPORT interaction_create_t : public event_dispatch_t {
   ...
   */
  void reply(interaction_response_type t, const message &m,
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void reply(interaction_response_type t, const std::string &mt,
             command_completion_event_t callback = {}) const;

  /**
   * @brief Send a reply for this interaction.
   * Uses the default type of dpp::ir_channel_message_with_source, a simple
   * message reply.
   *
   * @param m Message object to send. Not all fields are supported by Discord.
   * @param callback User function to execute when the api call completes.
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void reply(const std::string &mt,
             command_completion_event_t callback = {}) const;
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void dialog(const interaction_modal_response &mr,
              command_completion_event_t callback = {}) const;
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void edit_response(const message &m,
                     command_completion_event_t callback = {}) const;
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void edit_response(const std::string &mt,
                     command_completion_event_t callback = {}) const;
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void thinking(bool ephemeral = false,
                command_completion_event_t callback = {}) const;
   ...
   */
  [[nodiscard]] const virtual command_value &
...
/**
 * @brief Discord requests that we fill a list of auto completion choices for a
 * command option
 */
struct DPP_EXPORT autocomplete_t : public interaction_create_t {
   ...
   *
   * @param name The command line parameter to retrieve
   * @return Always returns an empty parameter as auto complete requests dont
   * have parameters!
   */
  [[nodiscard]] const command_value &
  get_parameter(const std::string &name) const override;
...
struct DPP_EXPORT guild_join_request_delete_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_join_request_delete_t(class discord_client *client,
                              const std::string &raw);
  /** Deleted guild */
  snowflake guild_id;
  /**
...
struct DPP_EXPORT message_reaction_remove_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  message_reaction_remove_t(class discord_client *client,
                            const std::string &raw);
  /**
   * @brief Guild reaction occurred on
   */
...
struct DPP_EXPORT message_reaction_remove_emoji_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  message_reaction_remove_emoji_t(class discord_client *client,
                                  const std::string &raw);
  /**
   * @brief Guild reaction occurred on
   */
...
struct DPP_EXPORT message_reaction_remove_all_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  message_reaction_remove_all_t(class discord_client *client,
                                const std::string &raw);
  /**
   * @brief Guild reaction occurred on
   */
...
struct DPP_EXPORT guild_integrations_update_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  guild_integrations_update_t(class discord_client *client,
                              const std::string &raw);
  /**
...
struct DPP_EXPORT message_create_t : public event_dispatch_t {
   ...
   * @note confirmation_callback_t::value contains a message object on success.
   * On failure, value is undefined and confirmation_callback_t::is_error() is
   * true.
   */
  void send(const std::string &m,
            command_completion_event_t callback = {}) const;
  /**
   * @brief Send a message to the same channel as the channel_id in received
   * event.
   ...
   * @param m Text to send
   * @param mention_replied_user mentions (pings) the author of message replied
   * to, if true
   * @param callback User function to execute once the API call completes.
   * @note confirmation_callback_t::value contains a message object on success.
   * On failure, value is undefined and confirmation_callback_t::is_error() is
   * true.
   */
  void reply(const std::string &m, bool mention_replied_user = false,
             command_completion_event_t callback = {}) const;
  /**
   * @brief Reply to the message received in the event.
   ...
   * true.
   */
  void reply(message &msg, bool mention_replied_user = false,
             command_completion_event_t callback = {}) const;
  /**
   * @brief Reply to the message received in the event.
   * @param msg Message to send as a reply.
...
struct DPP_EXPORT voice_client_disconnect_t : public event_dispatch_t {
   ...
   * @param raw Raw event text as JSON
   */
  voice_client_disconnect_t(class discord_client *client,
                            const std::string &raw);
  /**
