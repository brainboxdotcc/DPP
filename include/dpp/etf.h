...
[[maybe_unused]] const uint8_t FORMAT_VERSION = 131;
...
enum etf_token_type : uint8_t
{
  ...
  /// 98  [Int32:Int]
  ett_integer = 'b',
  /// 99  [31:Float String] Float in string format (formatted "%.20e", sscanf
  /// "%lf"). Superseded by ett_new_float
  ett_float = 'c',
  ...
  ett_bigint_large = 'o',
  /// 112 [UInt32:Size, UInt8:Arity, 16*Uint6-MD5:Uniq, UInt32:Index,
  /// UInt32:NumFree, atom:Module, int:OldIndex, int:OldUniq, pid:Pid,
  /// NunFree*ext:FreeVars]
  ett_new_function = 'p',
  ...
  ett_map = 't',
  /// 117 [UInt4:NumFree, pid:Pid, atom:Module, int:Index, int:Uniq,
  /// NumFree*ext:FreeVars]
  ett_function = 'u',
  /// 118 [UInt16:Len, Len:AtomName] max Len is 255 characters (up to 4 bytes
  /// per)
  ett_atom_utf8 = 'v',
...
/**
 * @brief A horrible structure used within the ETF parser to convert uint64_t to
 * double and back. This is horrible, but it is the official way erlang term
 * format does this, so we can't really mess with it much.
 */
union type_punner {
  /**
...
/**
 * @brief The etf_parser class can serialise and deserialise ETF (Erlang Term
 * Format) into and out of an nlohmann::json object, so that layers above the
 * websocket don't have to be any different for handling ETF.
 */
class DPP_EXPORT etf_parser
{
   ...
   * Erlang expects this to be supported, in practice Discord doesn't send these
   * right now.
   *
   * @return nlohmann::json value converted to JSON
   * @throw dpp::exception Data stream isn't long enough to fetch requested bits
