...
struct DPP_EXPORT scheduled_event : public managed {
  ...
  snowflake channel_id; //!< the channel id in which the scheduled event will be
                        //!< hosted, or null if scheduled entity type is
                        //!< EXTERNAL (may be empty)
  snowflake creator_id; //!< Optional: the id of the user that created the
                        //!< scheduled event
  ...
  time_t scheduled_end_time; //!< the time the scheduled event will end, or null
                             //!< if the event does not have a scheduled time to
                             //!< end (may be empty)
  event_privacy_level
	  privacy_level;   //!< the privacy level of the scheduled event
  event_status status; //!< the status of the scheduled event
  event_entity_type
	  entity_type; //!< the type of hosting entity associated with a scheduled
                   //!< event, e.g. voice channel or stage channel
  snowflake entity_id; //!< any additional id of the hosting entity associated
                       //!< with event, e.g. stage instance id) (may be empty)
  event_entities entity_metadata; //!< the entity metadata for the scheduled
                                  //!< event (may be empty)
  user creator;        //!< Optional: the creator of the scheduled event
  uint32_t user_count; //!< Optional: the number of users subscribed to the
                       //!< scheduled event
   ...
   */
  ~scheduled_event() override = default;
  ...
  build_json(bool with_id = false) const;
};

/**
 * @brief A group of scheduled events
 */
using scheduled_event_map = std::unordered_map<snowflake, scheduled_event>;

/**
 * @brief A group of scheduled event members
 */
using event_member_map = std::unordered_map<snowflake, event_member>;
