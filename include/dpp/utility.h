 ...
 *
 * @note These values are the actual character values specified by the Discord
 * API and should not be changed unless the Discord API changes the
 * specification! They have been sorted into numerical order of their ASCII
 * value to keep C++ happy.
 */
 ...
 */
using cmd_result_t = std::function<void(const std::string &)>;
 ...
 * stdout is passed to the callback function in its string parameter. For
 * example
 * ```
 ...
 * @param parameters Command line parameters. Each will be escaped using
 * `std::quoted`.
 * @param callback The callback to call on completion.
 */
void DPP_EXPORT exec(const std::string &cmd,
                     std::vector<std::string> parameters = {},
                     cmd_result_t callback = {});

/**
 * @brief Return a mentionable timestamp (used in a discord embed). These
 ...
 * @return std::string Substring in UTF-8 or empty string if invalid UTF-8
 * passed in
 */
std::string DPP_EXPORT utf8substr(const std::string &str,
                                  std::string::size_type start,
                                  std::string::size_type length);

/**
 * @brief Read a whole file into a std::string.
 * Be sure you have enough memory to read the file, if you are reading a large
 * file.
 * @note Be aware this function can block! If you are regularly reading large
 * files, consider caching them.
 ...
 * In the event the length of the string is less than _min, then an exception of
 * type dpp:length_exception will be thrown. If the string is longer than _max
 * UTF8 codepoints it will be truncated to fit.
 *
 ...
 */
std::string DPP_EXPORT validate(const std::string &value, size_t _min,
                                size_t _max,
 ...
 */
std::vector<std::string> DPP_EXPORT tokenize(std::string const &in,
                                             const char *sep = "\r\n");
 ...
 * @return Invite URL
 */
std::string DPP_EXPORT bot_invite_url(const snowflake bot_id,
 ...
 * This means that you can still use a code block, and the text within will be
 * left as-is. If set to true, code blocks will also be escaped so that ` symbol
 * may be used as a normal character.
 * @return std::string escaped text
 */
std::string DPP_EXPORT markdown_escape(const std::string &text,
...
/**
 * @brief Build a URL parameter string e.g. "a=b&c=d&e=f" from a map of
 * key/value pairs. Entries with empty key names or values are omitted.
 *
 * @param parameters parameters to create a url query string for
 * @return std::string A correctly encoded url query string
 */
std::string DPP_EXPORT
make_url_parameters(const std::map<std::string, std::string> &parameters);

/**
 * @brief Build a URL parameter string e.g. "a=b&c=d&e=f" from a map of
 * key/value pairs. Entries with empty key names or zero values are omitted.
 *
 * @param parameters parameters to create a url query string for
 * @return std::string A correctly encoded url query string
 */
std::string DPP_EXPORT
make_url_parameters(const std::map<std::string, uint64_t> &parameters);
