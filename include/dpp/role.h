...
enum role_permissions : uint64_t {
	p_create_instant_invite	=	(uint64_t)(1<<0),	//!< allows creationboosters of instant invites
	p_kick_members		=	(uint64_t)(1<<1),	//!< allows kicking members
	p_ban_members		=	(uint64_t)(1<<2),	//!< allows banning members
	p_administrator		=	(uint64_t)(1<<3),	//!< allows all permissions and bypasses channel permission overwrites
	p_manage_channels	=	(uint64_t)(1<<4),	//!< allows management and editing of channels
	p_manage_guild		=	(uint64_t)(1<<5),	//!< allows management and editing of the guild
	p_add_reactions		=	(uint64_t)(1<<6),	//!< allows for the addition of reactions to messages
	p_view_audit_log	=	(uint64_t)(1<<7),	//!< allows for viewing of audit logs
	p_priority_speaker	=	(uint64_t)(1<<8),	//!< allows for using priority speaker in a voice channel
	p_stream		=	(uint64_t)(1<<9),	//!< allows the user to go live
	p_view_channel		=	(uint64_t)(1<<10),	//!< allows guild members to view a channel, which includes reading messages in text channels
	p_send_messages		=	(uint64_t)(1<<11),	//!< allows for sending messages in a channel
	p_send_tts_messages	=	(uint64_t)(1<<12),	//!< allows for sending of /tts messages
	p_manage_messages	=	(uint64_t)(1<<13),	//!< allows for deletion of other users messages
	p_embed_links		=	(uint64_t)(1<<14),	//!< links sent by users with this permission will be auto-embedded
	p_attach_files		=	(uint64_t)(1<<15),	//!< allows for uploading images and files
	p_read_message_history	=	(uint64_t)(1<<16),	//!< allows for reading of message history
	p_mention_everyone	=	(uint64_t)(1<<17),	//!< allows for using the @everyone and the @here tag to notify users in a channel
	p_use_external_emojis	=	(uint64_t)(1<<18),	//!< allows the usage of custom emojis from other servers
	p_view_guild_insights	=	(uint64_t)(1<<19),	//!< allows for viewing guild insights
	p_connect		=	(uint64_t)(1<<20),	//!< allows for joining of a voice channel
	p_speak			=	(uint64_t)(1<<21),	//!< allows for speaking in a voice channel
	p_mute_members		=	(uint64_t)(1<<22),	//!< allows for muting members in a voice channel
	p_deafen_members	=	(uint64_t)(1<<23),	//!< allows for deafening of members in a voice channel
	p_move_members		=	(uint64_t)(1<<24),	//!< allows for moving of members between voice channels
	p_use_vad		=	(uint64_t)(1<<25),	//!< allows for using voice-activity-detection in a voice channel
	p_change_nickname	=	(uint64_t)(1<<26),	//!< allows for modification of own nickname 
	p_manage_nicknames	=	(uint64_t)(1<<27),	//!< allows for modification of other users nicknames 
	p_manage_roles		=	(uint64_t)(1<<28),	//!< allows management and editing of roles 
	p_manage_webhooks	=	(uint64_t)(1<<29),	//!< allows management and editing of webhooks
	p_manage_emojis		=	(uint64_t)(1<<30),	//!< allows management and editing of emojis 
	p_use_slash_commands	=	(uint64_t)(1<<31),	//!< allows members to use slash commands in text channels
	p_request_to_speak	=	(uint64_t)(1<<32),	//!< allows for requesting to speak in stage channels. (Discord: This permission is under active development and may be changed or removed.)
	p_manage_threads	=	(uint64_t)(1<<34),	//!< allows for deleting and archiving threads, and viewing all private threads
	p_use_public_threads	=	(uint64_t)(1<<35),	//!< allows for creating and participating in thread
	p_use_private_threads	=	(uint64_t)(1<<36),	//!< allows for creating and participating in private thread
};
