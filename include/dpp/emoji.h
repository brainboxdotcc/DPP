...
public:
   ...
   */
  [[nodiscard]] bool requires_colons() const;
   ...
   * @throw dpp::exception Image content exceeds discord maximum of 256
   * kilobytes
   */
  emoji &load_image(const std::string &image_blob, const image_type type);

  /**
   * @brief Format to name if unicode, name:id if has id or a:name:id if
   * animated
   *
 ...
 */
using emoji_map = std::unordered_map<snowflake, emoji>;
