 ...
 */
using channel_collector_t = dpp::collector<dpp::channel_create_t, dpp::channel>;

/**
 * @brief Template type for base class of thread collector
 */
using thread_collector_t = dpp::collector<dpp::thread_create_t, dpp::thread>;

/**
 * @brief Template type for base class of role collector
 */
using role_collector_t = dpp::collector<dpp::guild_role_create_t, dpp::role>;

/**
 * @brief Template type for base class of scheduled event collector
 */
using scheduled_event_collector_t =
	dpp::collector<dpp::guild_scheduled_event_create_t, dpp::scheduled_event>;

/**
 * @brief Template type for base class of message collector
 */
using message_collector_t = dpp::collector<dpp::message_create_t, dpp::message>;

/**
 * @brief Template type for base class of message reaction collector
 */
using reaction_collector_t =
	dpp::collector<dpp::message_reaction_add_t, dpp::collected_reaction>;

/**
 * @brief Message collector.
 * Collects messages during a set timeframe and returns them in a list via the
 * completed() method.
 */
...
public:
  ...
  message_collector(cluster *cl, uint64_t duration)
	  : message_collector_t::collector(cl, duration, cl->on_message_create) {}
   ...
   */
  void completed(const std::vector<dpp::message> &list) override = 0;
   ...
   * element
   */
  const dpp::message *filter(const dpp::message_create_t &element) override {
	return &element.msg;
  }
   ...
   */
  ~message_collector() override = default;
};

/**
 * @brief Reaction collector.
 * Collects message reactions during a set timeframe and returns them in a list
 * via the completed() method.
 */
...
public:
   ...
   * @param cl cluster to associate the collector with
   * @param duration Duration of time to run the collector for in seconds
   * @param msg_id Optional message ID. If specified, only collects reactions
   * for the given message
   */
  reaction_collector(cluster *cl, uint64_t duration, snowflake msg_id = 0)
	  : reaction_collector_t::collector(cl, duration,
                                        cl->on_message_reaction_add),
		message_id(msg_id) {}
   ...
   * @param list items collected during the timeframe specified
   */
  void completed(const std::vector<dpp::collected_reaction> &list) override = 0;
   ...
   * element
   */
  const dpp::collected_reaction *
  filter(const dpp::message_reaction_add_t &element) override {
   ...
   */
  ~reaction_collector() override = default;
};

/**
 * @brief Channel collector.
 * Collects channels during a set timeframe and returns them in a list via the
 * completed() method.
 */
...
public:
   ...
   * @param duration Duration of time to run the collector for in seconds
   */
  channel_collector(cluster *cl, uint64_t duration)
	  : channel_collector_t::collector(cl, duration, cl->on_channel_create) {}
   ...
   * @param list items collected during the timeframe specified
   */
  void completed(const std::vector<dpp::channel> &list) override = 0;
   ...
   * element
   */
  const dpp::channel *filter(const dpp::channel_create_t &element) override {
   ...
   */
  ~channel_collector() override = default;
};

/**
 * @brief Thread collector.
 * Collects threads during a set timeframe and returns them in a list via the
 * completed() method.
 */
...
public:
   ...
   * @param duration Duration of time to run the collector for in seconds
   */
  thread_collector(cluster *cl, uint64_t duration)
	  : thread_collector_t::collector(cl, duration, cl->on_thread_create) {}
   ...
   * @param list items collected during the timeframe specified
   */
  void completed(const std::vector<dpp::thread> &list) override = 0;
   ...
   * element
   */
  const dpp::thread *filter(const dpp::thread_create_t &element) override {
	return &element.created;
  }
   ...
   */
  ~thread_collector() override = default;
};

/**
 * @brief Role collector.
 * Collects guild roles during a set timeframe and returns them in a list via
...
public:
  ...
  role_collector(cluster *cl, uint64_t duration)
	  : role_collector_t::collector(cl, duration, cl->on_guild_role_create) {}
   ...
   * @param list items collected during the timeframe specified
   */
  void completed(const std::vector<dpp::role> &list) override = 0;
   ...
   * element
   */
  const dpp::role *filter(const dpp::guild_role_create_t &element) override {
	return element.created;
  }
   ...
   */
  ~role_collector() override = default;
};
...
public:
   ...
   * @param duration Duration of time to run the collector for in seconds
   */
  scheduled_event_collector(cluster *cl, uint64_t duration)
	  : scheduled_event_collector_t::collector(
			cl, duration, cl->on_guild_scheduled_event_create) {}
   ...
   * @param list items collected during the timeframe specified
   */
  void completed(const std::vector<dpp::scheduled_event> &list) override = 0;
   ...
   * element
   */
  const dpp::scheduled_event *
  filter(const dpp::guild_scheduled_event_create_t &element) override {
	return &element.created;
  }
   ...
   */
  ~scheduled_event_collector() override = default;
};
