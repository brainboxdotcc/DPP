...
enum channel_type
{
  ...
  GUILD_NEWS = 5, //!< a channel that users can follow and crosspost into their
                  //!< own server
  /**
  ...
  GUILD_STAGE = 13 //!< a "stage" channel, like a voice channel with one
                   //!< authorised speaker
};
...
using thread_member_map = std::unordered_map<snowflake, thread_member>;
...
public:
   ...
   * it contains the calculated permission bitmask of the user issuing the
   * command within this channel.
   */
   ...
   * @param allowed_permissions bitmask of allowed permissions (refer to enum
   * role_permissions) for this user/role in this channel
   * @param denied_permissions bitmask of denied permissions (refer to enum
   * role_permissions) for this user/role in this channel
   *
   * @return Reference to self, so these method calls may be chained
   */
  channel &add_permission_overwrite(const snowflake id, const uint8_t type,
  ...
  /**
   * @brief Get the channel's banner url if they have one, otherwise returns an
   * empty string
   *
  ...
  /**
   * @brief Get the channel's icon url (if its a group DM), otherwise returns an
   * empty string
   *
   ...
   */
  [[nodiscard]] bool is_text_channel() const;
   ...
   */
  [[nodiscard]] bool is_voice_channel() const;
   ...
   */
  [[nodiscard]] bool is_group_dm() const;
   ...
   */
  [[nodiscard]] bool is_news_channel() const;
   ...
   */
  [[nodiscard]] bool is_store_channel() const;
   ...
   */
  [[nodiscard]] bool is_stage_channel() const;
   ...
   */
  [[nodiscard]] bool is_video_auto() const;
   ...
   */
  [[nodiscard]] bool is_video_720p() const;
};
...
public:
   ...
   */
  [[nodiscard]] bool is_news_thread() const;
   ...
   */
  [[nodiscard]] bool is_public_thread() const;
   ...
   */
  [[nodiscard]] bool is_private_thread() const;
 ...
 */
using thread_map = std::unordered_map<snowflake, thread>;
