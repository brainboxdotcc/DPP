 ...
 * A client can only connect to one voice channel per guild at a time, so these
 * are stored in a map in the dpp::discord_client keyed by guild_id.
 */
...
/** @brief Implements a discord client. Each discord_client connects to one
 * shard and derives from a websocket client. */
class DPP_EXPORT discord_client : public websocket_client
{
protected:
  /**
   * @brief Needed so that voice_state_update can call
   * dpp::discord_client::disconnect_voice_internal
   */
  friend class dpp::events::voice_state_update;

  /**
   * @brief Needed so that guild_create can request member chunks if you have
   * the correct intents
   */
   ...
   * @param send_json True if we should send a json message confirming we are
   * leaving the VC Should be set to false if we already receive this message in
   * an event.
   */
...
public:
   ...
   */
  virtual void handle_event(const std::string &event, json &j,
   ...
   *
   * @return dpp::utility::uptime Detail of how long the shard has been
   * connected for
   */
   ...
   * @param intents Privileged intents to use, a bitmask of values from
   * dpp::intents
   * @param compressed True if the received data will be gzip compressed
   * @param ws_protocol Websocket protocol to use for the connection, JSON or
   * ETF
   */
  discord_client(dpp::cluster *_cluster, uint32_t _shard_id,
                 uint32_t _max_shards, const std::string &_token,
                 uint32_t intents = 0, bool compressed = true,
  ...
  void connect_voice(snowflake guild_id, snowflake channel_id,
                     bool self_mute = false, bool self_deaf = false);
   ...
   * @return voiceconn* The voice connection for the guild, or nullptr if there
   * is no voice connection to this guild.
   */
