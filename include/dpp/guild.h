...
enum guild_flags : uint32_t
{
  ...
  /** Guild has an actual animated icon (set by the icon hash starting with
     'a_') */
  g_has_animated_icon = 0b00000000000100000000000000000000,
  /** Guild has an actual animated banner (set by the icon hash starting with
     'a_') */
...
/**
 * @brief Various flags that can be used to indicate the status of a guild
 * member
 */
 ...
 * This contains the user's nickname, guild roles, and any other guild-specific
 * flags.
 */
...
public:
   ...
   */
  guild_member &fill_from_json(nlohmann::json *j, snowflake g_id,
                               snowflake u_id);
   ...
   */
  [[nodiscard]] std::string build_json(bool with_id = false) const;
   ...
   */
  [[nodiscard]] bool is_communication_disabled() const;
   ...
   */
  [[nodiscard]] bool has_animated_guild_avatar() const;

  /**
   * @brief Returns the members per guild avatar if they have one, otherwise
   * returns an empty string
   *
   * @note per-server avatar is a nitro only feature so it might be not set. If
   * you need the real user avatar, use user::get_avatar_url.
   *
   * @param size The size of the avatar in pixels. It can be any power of two
   * between 16 and 4096. If not specified, the default sized avatar is
   * returned.
   * @return std::string avatar url or empty string
   */
  [[nodiscard]] std::string get_avatar_url(uint16_t size = 0) const;
 ...
 * Used to represent just how naughty this guild is. Naughty  guild, go sit in
 * the corner.
 * @note This is set by Discord, and cannot be set by any bot or user on the
 * guild.
 */
 ...
 * This is set by a guild admin, but can be forced to a setting if the server is
 * verified, partnered, official etc.
 */
...
/**
 * @brief MFA level for server. If set to elevated all moderators need MFA to
 * perform specific actions such as kick or ban.
 */
 ...
 */
using members_container = std::unordered_map<snowflake, guild_member>;
...
public:
   ...
   * @note This field cannot be set from the API. Attempts to change this value
   * will be silently ignored even if the correct number of boosts or
   * verified/partnered status exist. See:
   * https://github.com/discord/discord-api-docs/issues/519
   */
   ...
   * @note Generally Discord always fills this with 0, apart from for the
   * largest of guilds
   */
   ...
   */
  uint64_t permission_overwrites(const uint64_t base_permissions,
                                 const user *member,
   ...
   */
  bool connect_member_voice(snowflake user_id, bool self_mute = false,
                            bool self_deaf = false);

  /**
   * @brief Get the banner url of the guild if it have one, otherwise returns an
   * empty string
   *
   * @param size The size of the banner in pixels. It can be any power of two
   * between 16 and 4096. if not specified, the default sized banner is
   * returned.
  ...
  /**
   * @brief Get the discovery splash url of the guild if it have one, otherwise
   * returns an empty string
   *
   * @param size The size of the discovery splash in pixels. It can be any power
   * of two between 16 and 4096. if not specified, the default sized discovery
   * splash is returned.
  ...
  /**
   * @brief Get the icon url of the guild if it have one, otherwise returns an
   * empty string
   *
  ...
  /**
   * @brief Get the splash url of the guild if it have one, otherwise returns an
   * empty string
   *
   * @param size The size of the splash in pixels. It can be any power of two
   * between 16 and 4096. if not specified, the default sized splash is
   * returned.
  ...
  /**
   * @brief Is unavailable due to outage (most other fields will be blank or
   * outdated
...
public:
   ...
   */
  [[nodiscard]] virtual std::string build_json(bool with_id = false) const;
};
 ...
 *
 * @throw dpp::cache_exception if the guild or guild_member is not found in the
 * cache
 * @return guild_member the cached object, if found
 */
guild_member DPP_EXPORT find_guild_member(const snowflake guild_id,
