...
namespace dpp {
/**
 * @brief Represents an interface for an object that can optionally implement
 * functions for converting to and from nlohmann::json. In the event either
 * parse_from_json() or build_json() are not implemented and are called, they
...
struct DPP_EXPORT json_interface {
  ...
  T &fill_from_json(nlohmann::json *j) {
	throw dpp::logic_exception(
		"JSON interface doesn't implement parse_from_json");
  }
