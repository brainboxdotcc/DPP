...
public:
   ...
   *
   * The object must be derived from dpp::managed and should be allocated on the
   * heap. Generally this is done via `new`. Once stored in the cache the
   * lifetime of the stored object is managed by the cache class unless the
   * cache is deleted (at which point responsibility for deleting the object
   * returns to its allocator). Objects stored are removed when the
   * cache::remove() method is called by placing them into a garbage collection
   * queue for deletion within the next 60 seconds, which are then deleted in
   * bulk for efficiency and to aid thread safety.
   *
   * @note Adding an object to the cache with an ID which already exists
   * replaces that entry. The previously entered cache item is inserted into the
   * garbage collection queue for deletion similarly to if cache::remove() was
   * called first.
   *
   * @param object object to store. Storing a pointer to the cache relinquishes
   * ownership to the cache object.
   */
  void store(T *object) {
	if (!object) {
	  return;
   ...
   *
   * @note The cache class takes ownership of the pointer, and calling this
   * method will cause deletion of the object within the next 60 seconds by
   * means of a garbage collection queue. This queue aids in efficiency by
   * freeing memory in bulk, and assists in thread safety by ensuring that all
   * deletions can be locked and freed at the same time.
   *
   ...
   *
   * The cache is searched for the object. All dpp::managed objects have a
   * snowflake id (this is the only field dpp::managed actually has).
   *
   * @warning Do not hang onto objects returned by cache::find() indefinitely.
   * They may be deleted at a later date if cache::remove() is called. If
   * persistence is required, take a copy of the object after checking its
   * pointer is non-null.
   *
  ...
  void rehash() {
	std::unique_lock l(cache_mutex);
	auto n = new std::unordered_map<snowflake, T *>;
