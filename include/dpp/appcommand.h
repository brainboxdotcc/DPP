...
/**
 * @brief Discord limits the maximum number of replies to an autocomplete
 * interaction to 25. This value represents that maximum.
 * interaction_response::add_autocomplete_choice does not allow adding more than
 * this number of elements to the vector.
 */
 ...
 *
 * std::monostate indicates an invalid parameter value, e.g. an unfilled
 * optional parameter.
 */
using command_value =
...
struct DPP_EXPORT command_option_choice
	: public json_interface<command_option_choice> {
...
/**
 * @brief A minimum or maximum value for co_number and co_integer
 * dpp::command_option types
 */
using command_option_range = std::variant<std::monostate, int64_t, double>;
...
struct DPP_EXPORT command_option : public json_interface<command_option> {
  ...
  bool focused; //!< True if the user is typing in this field, when sent via
                //!< autocomplete
  command_value
	  value; //!< Set only by autocomplete went sent as part of an interaction
  std::vector<command_option_choice>
	  choices;       //!< List of choices for multiple choice command
  ...
  std::vector<channel_type>
	  channel_types; //!< Allowed channel types for channel snowflake id options
  command_option_range min_value; //!< Minimum value allowed, for co_number and
                                  //!< co_integer types only
  command_option_range max_value; //!< Maximum value allowed, for co_number and
                                  //!< co_integer types only
   ...
   */
  command_option(command_option_type t, const std::string &name,
   ...
   * @throw dpp::exception command_option is an autocomplete, so choices cannot
   * be added
   */
   ...
   * @return command_option& return a reference to self for chaining of calls
   * @throw dpp::exception You attempted to enable auto complete on a
   * command_option that has choices added to it
   */
...
/**
 * @brief Response types when responding to an interaction within
 * on_interaction_create. Do not use ir_acknowledge or ir::channel_message, as
 * these are deprecated in the Discord API spec. They are listed in this enum
 * for completeness.
 */
enum interaction_response_type
{
  ...
  ir_deferred_channel_message_with_source =
	  5, //!< ACK an interaction and edit a response later, the user sees a
         //!< loading state
  ir_deferred_update_message =
	  6, //!< for components, ACK an interaction and edit the original message
         //!< later; the user does not see a loading state
  ir_update_message =
	  7, //!< for components, edit the message the component was attached to
  ir_autocomplete_reply = 8, //!< Reply to autocomplete interaction. Be sure to
                             //!< do this within 500ms of the interaction!
...
struct DPP_EXPORT interaction_response
	: public json_interface<interaction_response> {
   ...
   */
  interaction_response &
  add_autocomplete_choice(const command_option_choice &achoice);
 ...
 *
 * A dialog box is a modal popup which appears to the user instead of a message.
 * One or more components are displayed on a form (the same component structure
 * as within a dpp::message). When the user submits the form an on_form_submit
 * event is dispatched to any listeners.
 */
struct DPP_EXPORT interaction_modal_response
	: public interaction_response,
	  public json_interface<interaction_modal_response> {
private:
  size_t current_row;
...
public:
   ...
   * @param _title Title of the modal form. It will be truncated to the maximum
   * length of 45 UTF-8 characters.
   * @param _components Components to add to the modal form
   */
  interaction_modal_response(const std::string &_custom_id,
   ...
   * @throw dpp::logic_exception if more than five rows are attempted to be
   * added
   * @return interaction_modal_response& Reference to self
   */
...
struct DPP_EXPORT command_resolved {
  ...
  /**
   * @brief Resolved total guild member permissions in the channel, including
   * overwrites
   */
...
struct DPP_EXPORT command_data_option {
  ...
  std::vector<command_data_option>
	  options;  //!< Optional: present if this option is a group or subcommand
  bool focused; //!< Optional: true if this option is the currently focused
...
struct DPP_EXPORT command_interaction {
  ...
  std::vector<command_data_option>
	  options; //!< Optional: the params + values from the user
  slashcommand_contextmenu_type type; //!< type of the command interaction
  dpp::snowflake
	  target_id; //!< Non-zero target ID for context menu actions. e.g. user id
                 //!< or message id whom clicked or tapped with the context menu
...
public:
  ...
  guild_member member;  //!< Optional: guild member data for the invoking user,
  ...
  std::string
	  guild_locale; //!< Guild's locale (language) - for guild interactions only
   ...
   */
  [[nodiscard]] command_interaction get_command_interaction() const;
   ...
   */
  [[nodiscard]] component_interaction get_component_interaction() const;
   ...
   */
  [[nodiscard]] autocomplete_interaction get_autocomplete_interaction() const;
...
public:
   ...
   */
  command_permission(snowflake id, const command_permission_type t,
                     bool permission);
...
class DPP_EXPORT guild_command_permissions
	: public json_interface<guild_command_permissions>
{
public:
  snowflake id; //!< the id of the command
  snowflake
	  application_id; //!< the id of the application the command belongs to
  snowflake guild_id; //!< the id of the guild
  std::vector<command_permission>
...
class DPP_EXPORT slashcommand : public managed,
								public json_interface<slashcommand>
{
public:
  /**
  ...
  /**
   * @brief whether the command is enabled by default when the app is added to a
   * guild
   */
  ...
  /**
   * @brief autoincrementing version identifier updated during substantial
   * record changes
   */
  snowflake version;
   ...
   */
  slashcommand(const std::string &_name, const std::string &_description,
 ...
 */
using slashcommand_map = std::unordered_map<snowflake, slashcommand>;

/**
 * @brief A group of guild command permissions
 */
using guild_command_permissions_map =
	std::unordered_map<snowflake, guild_command_permissions>;
