 ...
 */
using socket_callback_t = std::function<dpp::socket()>;

/**
 * @brief A socket notification callback
 */
using socket_notification_t = std::function<void()>;
...
public:
  ...
  /**
   * @brief Attaching an additional file descriptor to this function will send
   * notifications when there is data to read.
   *
   * NOTE: Only hook this if you NEED it as it can increase CPU usage of the
   * thread! Returning -1 means that you don't want to be notified.
   */
  socket_callback_t custom_readable_fd;

  /**
   * @brief Attaching an additional file descriptor to this function will send
   * notifications when you are able to write to the socket.
   *
   * NOTE: Only hook this if you NEED it as it can increase CPU usage of the
   * thread! You should toggle this to -1 when you do not have anything to write
   * otherwise it'll keep triggering repeatedly (it is level triggered).
   */
   ...
   * without initialising SSL.
   * @param reuse Attempt to reuse previous connections for this hostname and
   * port, if available Note that no Discord endpoints will function when
   * downgraded. This option is provided only for connection to non-Discord
   * addresses such as within dpp::cluster::request().
   * @throw dpp::exception Failed to initialise connection
   */
   ...
   * @throw std::exception Any std::exception (or derivative) thrown from
   * read_loop() causes reconnection of the shard
   */
  void read_loop();
   ...
   * @param buffer the buffer content. Will be modified removing any processed
   * front elements
   */
