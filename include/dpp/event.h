...
#define event_decl(                                                               \
	x, wstype)    /** @brief Internal event handler for wstype websocket events.  \
                     Called for each websocket message of this type. @internal */ \
  class x : public event                                                          \
  {                                                                               \
  public:                                                                         \
	virtual void handle(dpp::discord_client *client, nlohmann::json &j,           \
...
/**
 * @brief The events namespace holds the internal event handlers for each
 * websocket event. These are handled internally and also dispatched to the user
 * code if the event is hooked.
 */
namespace events {

/**
 * @brief An event object represents an event handled internally, passed from
 * the websocket e.g. MESSAGE_CREATE.
 */
...
public:
   ...
   */
  virtual void handle(class discord_client *client, nlohmann::json &j,
