...
public:
  ...
  snowflake user_id;    //!< Optional: the user this webhook was created by (not
  ...
  std::string token;    //!< Optional: the secure token of the webhook (returned
                        //!< for Incoming Webhooks)
  snowflake application_id; //!< the bot/OAuth2 application that created this
                            //!< webhook (may be empty)
  std::string
	  *image_data; //!< base64 encoded image data if uploading a new image
  ...
  /**
   * @brief Construct a new webhook object using the Webhook URL provided by
   * Discord
   *
  ...
  /**
   * @brief Construct a new webhook object using the webhook ID and the webhook
   * token
   *
 ...
 */
using webhook_map = std::unordered_map<snowflake, webhook>;
