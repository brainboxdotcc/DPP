...
struct DPP_EXPORT select_option : public json_interface<select_option> {
   ...
   */
  select_option(const std::string &label, const std::string &value,
                const std::string &description = "");

  /**
   * @brief Set the label
   *
   * @param l the user-facing name of the option. It will be truncated to the
   * maximum length of 100 UTF-8 characters.
   ...
   * @brief Set the value
   *
   * @param v value to set. It will be truncated to the maximum length of 100
   * UTF-8 characters.
   * @return select_option& reference to self for chaining
   */
   ...
   *
   * @param d description to set. It will be truncated to the maximum length of
...
public:
   ...
   *
   * @param label Label text to set. It will be truncated to the maximum length
   * of 80 UTF-8 characters.
   * @return component& Reference to self
   */
   ...
   *
   * @param value Value text to set. It will be truncated to the maximum length
   * of 4000 UTF-8 characters.
   * @return component& Reference to self
   */
   ...
   *
   * @param url URL to set. It will be truncated to the maximum length of 512
   * UTF-8 characters.
   * @return component& reference to self.
   */
   ...
   * @return component& Reference to self
   */
  component &set_emoji(const std::string &name, dpp::snowflake id = 0,
                       bool animated = false);
...
struct DPP_EXPORT embed_footer {
   ...
   * @param t string to set as footer text. It will be truncated to the maximum
   * length of 2048 UTF-8 characters.
   * @return A reference to self
   */
  embed_footer &set_text(const std::string &t);

  /** Set footer's icon url. Returns footer itself so these methods may be
   * "chained"
  ...
  /** Set footer's proxied icon url. Returns footer itself so these methods may
   * be "chained"
...
/**
 * @brief Embed provider in a dpp::embed. Received from discord but cannot be
 * sent
 */
...
struct DPP_EXPORT embed {
  ...
  /** Set embed title. Returns the embed itself so these method calls may be
   * "chained"
   * @param text The text of the title. It will be truncated to the maximum
   * length of 256 UTF-8 characters.
   * @return A reference to self
   */
  embed &set_title(const std::string &text);

  /** Set embed description. Returns the embed itself so these method calls may
   * be "chained"
   * @param text The text of the title. It will be truncated to the maximum
   * length of 4096 UTF-8 characters.
   * @return A reference to self
   */
  embed &set_description(const std::string &text);

  /** Set the footer of the embed. Returns the embed itself so these method
  ...
  /** Set the footer of the embed. Returns the embed itself so these method
   * calls may be "chained"
   * @param text string to set as footer text. It will be truncated to the
   * maximum length of 2048 UTF-8 characters.
   * @param icon_url an url to set as footer icon url
   * @return A reference to self
   */
  embed &set_footer(const std::string &text, const std::string &icon_url);

  /** Set embed colour. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Set embed timestamp. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Set embed url. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Add an embed field. Returns the embed itself so these method calls may be
   * "chained"
   * @param name The name of the field. It will be truncated to the maximum
   * length of 256 UTF-8 characters.
   * @param value The value of the field. It will be truncated to the maximum
   * length of 1024 UTF-8 characters.
   * @param is_inline Whether or not to display the field 'inline' or on its own
   * line
   * @return A reference to self
   */
  embed &add_field(const std::string &name, const std::string &value,
                   bool is_inline = false);

  /** Set embed author. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Set embed author. Returns the embed itself so these method calls may be
   * "chained"
   * @param name The name of the author. It will be truncated to the maximum
   * length of 256 UTF-8 characters.
   * @param url The url of the author
   * @param icon_url The icon URL of the author
   * @return A reference to self
   */
  embed &set_author(const std::string &name, const std::string &url,
                    const std::string &icon_url);

  /** Set embed provider. Returns the embed itself so these method calls may be
   * "chained"
   * @param name The provider name. It will be truncated to the maximum length
   * of 256 UTF-8 characters.
   * @param url The provider url
   * @return A reference to self
   */
  embed &set_provider(const std::string &name, const std::string &url);

  /** Set embed image. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Set embed video. Returns the embed itself so these method calls may be
   * "chained"
  ...
  /** Set embed thumbnail. Returns the embed itself so these method calls may be
   * "chained"
...
struct DPP_EXPORT attachment {
   ...
   * @note The content of the file will be in the http_info.body parameter of
   * the callback parameter.
   * @throw dpp::exception If there is no owner associated with this attachment
   * that itself has an owning cluster, this method will throw a dpp::exception
   * when called.
   */
...
struct DPP_EXPORT sticker : public managed, public json_interface<sticker> {
  ...
  /// Optional: whether this guild sticker can be used, may be false due to loss
  /// of Server Boosts
  bool available;
   ...
   */
  [[nodiscard]] std::string build_json(bool with_id = true) const override;

  /**
   * @brief Get the sticker url
   *
   * @param accept_lottie Whether to allow that
   * [lottie](https://airbnb.io/lottie/#/) (json format) can be returned or not
   * @return std::string The sticker url or an empty string when its a lottie
   * and accept_lottie is false
   */
  [[nodiscard]] std::string get_url(bool accept_lottie = true) const;
...
/**
 * @brief Represents a sticker pack (the built in groups of stickers that all
 * nitro users get to use)
 */
struct DPP_EXPORT sticker_pack : public managed,
								 public json_interface<sticker_pack> {
...
{
  /**
   * @brief request aggressively on seeing new guilds, and also store missing
   * data from messages. This is the default behaviour and the least
   * memory-efficient option. Memory usage will increase over time, initially
   * quite rapidly, and then linearly over time. It is the least cpu-intensive
   * setting.
   */
  cp_aggressive = 0,
  /**
   * @brief only cache when there is relevant activity, e.g. a message to the
   * bot. This is a good middle-ground, memory usage will increase linearly over
   * time.
   */
  cp_lazy = 1,
  /**
...
struct DPP_EXPORT message : public managed {
   ...
   * Discord: Only textual channels that are visible to everyone in a lurkable
   * guild will ever be included. Only crossposted messages (via Channel
   * Following) currently include mention_channels at all. (includes ID, Guild
   * ID, Type, Name)*/
  ...
  bool pinned;
  /** Optional: if the message is generated by a webhook, its id will be here
   * otherwise the field will be 0 */
  ...
  struct allowed_ref {
	...
	bool parse_roles;
	/**
	 * @brief Set to true to mention the user who sent the message this one is
	 * replying to
	 */
	bool replied_user;
	/**
  ...
  /**
   * @brief Construct a new message object
   * @param o Owning cluster, passed down to various things such as
   * dpp::attachment. Owning cluster is optional (can be nullptr) and if nulled,
   * will prevent some functions such as attachment::download from functioning
   * (they will throw, if used)
   */
   ...
   * @param content The content of the message. It will be truncated to the
   * maximum length of 2000 UTF-8 characters.
   ...
   * @param fail_if_not_exists true if the message send should fail if these
   * values are invalid (optional)
   * @return message& reference to self
   */
  message &set_reference(snowflake _message_id, snowflake _guild_id = 0,
   ...
   *
   * @param _parse_users whether or not to parse users in the message content or
   * embeds
   * @param _parse_roles whether or not to parse roles in the message content or
   * embeds
   * @param _parse_everyone whether or not to parse everyone/here in the message
   * content or embeds
   * @param _replied_user if set to true and this is a reply, then ping the user
   * we reply to
   * @param users list of user ids to allow pings for
   * @param roles list of role ids to allow pings for
   * @return message& reference to self
   */
  message &set_allowed_mentions(bool _parse_users, bool _parse_roles,
                                bool _parse_everyone, bool _replied_user,
                                const std::vector<snowflake> &users,
                                const std::vector<snowflake> &roles);

  /** Fill this object from json.
   * @param j JSON object to fill from
   * @param cp Cache policy for user records, whether or not we cache users when
   * a message is received
   * @return A reference to self
   */
  message &fill_from_json(nlohmann::json *j,
                          cache_policy_t cp = {cp_aggressive, cp_aggressive,
                                               cp_aggressive});

  /** Build JSON from this object.
   * @param with_id True if the ID is to be included in the built JSON
   * @param is_interaction_response Set to true if this message is intended to
   * be included in an interaction response. This will exclude some fields that
   * are not valid in interactions at this time.
   * @return The JSON text of the message
   */
  [[nodiscard]] std::string
  build_json(bool with_id = false, bool is_interaction_response = false) const;
   ...
   */
  [[nodiscard]] bool is_crossposted() const;
   ...
   */
  [[nodiscard]] bool is_crosspost() const;
   ...
   */
  [[nodiscard]] bool suppress_embeds() const;
   ...
   */
  [[nodiscard]] bool is_source_message_deleted() const;
   ...
   * @return message& reference to self
   */
  message &add_file(const std::string &filename,
                    const std::string &filecontent);

  /**
   * @brief Set the message content
   *
   * @param c message content. It will be truncated to the maximum length of
...
/** A group of messages */
using message_map = std::unordered_map<snowflake, message>;

/** A group of stickers */
using sticker_map = std::unordered_map<snowflake, sticker>;

/** A group of sticker packs */
using sticker_pack_map = std::unordered_map<snowflake, sticker_pack>;
