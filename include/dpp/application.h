...
public:
  utility::iconhash
	  icon;     //!< a hash of the image of the team's icon (may be empty)
...
class DPP_EXPORT application : public managed,
							   public json_interface<application>
{
public:
  ...
  std::string
	  rpc_origins; //!< Optional: an array of rpc origin urls, if rpc is enabled
  bool bot_public; //!< when false only app owner can join the app's bot to
                   //!< guilds
  bool
	  bot_require_code_grant; //!< when true the app's bot will only join upon
                              //!< completion of the full oauth2 code grant flow
  std::string
	  terms_of_service_url; //!< Optional: the url of the app's terms of service
  std::string
	  privacy_policy_url; //!< Optional: the url of the app's privacy policy
  user owner; //!< Optional: partial user object containing info on the owner of
              //!< the application
  std::string summary; //!< if this application is a game sold on Discord, this
                       //!< field will be the summary field for the store page
                       //!< of its primary sku
  std::string verify_key; //!< the hex encoded key for verification in
                          //!< interactions and the GameSDK's GetTicket
  app_team team; //!< if the application belongs to a team, this will be a list
                 //!< of the members of that team (may be empty)
  snowflake
	  guild_id; //!< Optional: if this application is a game sold on Discord,
                //!< this field will be the guild to which it has been linked
  snowflake primary_sku_id; //!< Optional: if this application is a game sold on
                            //!< Discord, this field will be the id of the "Game
                            //!< SKU" that is created, if exists
  std::string
	  slug; //!< Optional: if this application is a game sold on Discord, this
            //!< field will be the URL slug that links to the store page
  utility::iconhash cover_image; //!< Optional: the application's default rich
 ...
 * objects that can be received by REST has the possibility for this, so this
 * exists. Don't ever expect to see one at present.
 */
using application_map = std::unordered_map<snowflake, application>;
