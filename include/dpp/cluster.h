...
public:
	 ...
	 */
	cluster(const std::string& token, uint32_t intents = i_default_intents, uint32_t shards = 0, uint32_t cluster_id = 0, uint32_t maxclusters = 1, bool compressed = true, cache_policy_t policy = { .user_policy = cp_aggressive, .emoji_policy = cp_aggressive, .role_policy = cp_aggressive, .channel_policy = cp_aggressive, .guild_policy = cp_aggressive }, uint32_t request_threads = 12, uint32_t request_threads_raw = 1);
