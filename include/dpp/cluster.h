 ...
 */
using shard_list = std::map<uint32_t, class discord_client *>;
 ...
 *
 */
using confirmable_t = std::variant<
	confirmation, message, message_map, user, user_identified, user_map,
	guild_member, guild_member_map, channel, channel_map, thread_member,
	thread_member_map, guild, guild_map, guild_command_permissions,
	guild_command_permissions_map, role, role_map, invite, invite_map,
	dtemplate, dtemplate_map, emoji, emoji_map, ban, ban_map, voiceregion,
	voiceregion_map, integration, integration_map, webhook, webhook_map, prune,
	guild_widget, gateway, interaction, interaction_response, auditlog,
	slashcommand, slashcommand_map, stage_instance, sticker, sticker_map,
	sticker_pack, sticker_pack_map, application, application_map, connection,
	connection_map, thread, thread_map, scheduled_event, scheduled_event_map,
	event_member, event_member_map>;
...
struct DPP_EXPORT confirmation_callback_t {
   ...
   */
  confirmation_callback_t(cluster *creator, const confirmable_t &_value,
                          const http_request_completion_t &_http);

  /**
   * @brief Returns true if the call resulted in an error rather than a
   * legitimate value in the confirmation_callback_t::value member.
   *
   * @return true There was an error who's details can be obtained by
   * get_error()
   ...
   * The error_info object contains the details of any REST error, if there is
   * an error (to find out if there is an error check
   * confirmation_callback_t::is_error())
   *
 ...
 */
using json_encode_t =
	std::function<void(json &, const http_request_completion_t &)>;
 ...
 *
 * Multiple listeners may attach to the event_router_t by means of operator().
 *Passing a lambda into operator() attaches to the event.
 *
 * Dispatchers of the event may call the event_router_t::call() method to cause
 *all listeners to receive the event.
 *
 * The event_router_t::empty() method will return true if there are no listeners
 *attached to the event_router_t (this can be used to save time by not
 *constructing objects that nobody will ever see).
 *
 * The event_router_t::detach() method removes an existing listener from the
 *event, using the event_handle ID returned by operator().
 *
 * This class is used by the library to route all websocket events to listening
 *code.
 *
 * Example:
 *
 ...
 * });
 *
 * // Construct a log_t and call the event (listeners will receive the log_t
 *object) log_t lt; lt.message = "foo"; my_event.call(lt);
 *
 * // Detach from an event using the handle returned by operator()
 * my_event.detach(id);
 * ```
 *
 * @tparam T type of single parameter passed to event lambda derived from
 *event_dispatch_t
 */
template <class T>
...
public:
   ...
   * @return false The ID is invalid (possibly already detached, or does not
   * exist)
   */
...
/** @brief The cluster class represents a group of shards and a command queue
 * for sending and receiving commands from discord via HTTP. You should usually
 * instantiate a cluster object at the very least to make use of the library.
 */
class DPP_EXPORT cluster
{
  ...
  /**
   * @brief queue system for arbitrary HTTP requests sent by the user to sites
   * other than Discord
   */
  ...
  /**
   * @brief Active shards on this cluster. Shard IDs may have gaps between if
...
public:
  /**
   * @brief Current bot token for all shards on this cluster and all commands
   * sent via HTTP
   */
  std::string token;
  ...
  /**
   * @brief The details of the bot user. This is assumed to be identical across
   * all shards in the cluster. Each connecting shard updates this information.
   */
  dpp::user me;
  ...
  /**
   * @brief Websocket mode for all shards in the cluster, either ws_json or
   * ws_etf. Production bots should use ETF, while development bots should use
   * JSON.
   */
   ...
   * @param token The bot token to use for all HTTP commands and websocket
   * connections
   * @param intents A bitmask of dpd::intents values for all shards on this
   * cluster. This is required to be sent for all bots with over 100 servers.
   * @param shards The total number of shards on this bot. If there are multiple
   * clusters, then (shards / clusters) actual shards will run on this cluster.
   * If you omit this value, the library will attempt to query the Discord API
   * for the correct number of shards to start.
   * @param cluster_id The ID of this cluster, should be between 0 and
   * MAXCLUSTERS-1
   * @param maxclusters The total number of clusters that are active, which may
   * be on separate processes or even separate machines.
   * @param compressed Whether or not to use compression for shards on this
   * cluster. Saves a ton of bandwidth at the cost of some CPU
   * @param policy Set the user caching policy for the cluster, either lazy
   * (only cache users/members when they message the bot) or aggressive (request
   * whole member lists on seeing new guilds too)
   * @throw dpp::exception Thrown on windows, if WinSock fails to initialise, or
   * on any other system if a dpp::request_queue fails to construct
   */
  cluster(const std::string &token, uint32_t intents = i_default_intents,
          uint32_t shards = 0, uint32_t cluster_id = 0,
          uint32_t maxclusters = 1, bool compressed = true,
          cache_policy_t policy = {cp_aggressive, cp_aggressive,
                                   cp_aggressive});
   ...
   * Generally ws_etf is faster, but provides less facilities for debugging
   * should something go wrong. It is recommended to use ETF in production and
   * JSON in development.
   *
   ...
   * This is set per-thread, so you must ensure that if you call this method,
   *your request that is associated with the reason happens on the same thread
   *where you set the reason. Once the next call is made, the audit log reason
   *is cleared for this thread automatically.
   *
   * Example:
   * ```
  ...
  /**
   * @brief Get the audit reason set for the next REST call to be made on this
   * thread. This is set per-thread, so you must ensure that if you call this
   * method, your request that is associated with the reason happens on the same
   * thread where you set the reason. Once the next call is made, the audit log
   * reason is cleared for this thread automatically.
   *
   ...
   * The logged message is passed up the chain to the on_log event in user code
   * which can then do whatever it wants to do with it.
   ...
   * @param on_stop The callback lambda to call for this timer when it is
   * stopped
   ...
   * @note If the timer has an on_stop lambda, the on_stop lambda will be
   * called.
   */
   ...
   *
   * @param return_after If true the bot will return to your program after
   * starting shards, if false this function will never return.
   */
   ...
   *
   * @param p The presence to set. Only the online status and the first activity
   * are sent.
   */
   ...
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_state_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_client_disconnect_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_client_speaking_t&, and returns void.
   */
   ...
   * You can attach any logging system here you wish, e.g. spdlog, or even just
   * a simple use of std::cout or printf. If nothing attaches this log event,
   * then the library will be silent.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type log_t&, and returns
   * void.
   */
   ...
   * Triggered when a user declines the membership screening questionnaire for a
   * guild.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_join_request_delete_t&, and returns void.
   */
  event_router_t<guild_join_request_delete_t> on_guild_join_request_delete;

  /**
   * @brief Called when a new interaction is created.
   * Interactions are created by discord when commands you have registered are
   * issued by a user. For an example of this in action please see \ref
   * slashcommands
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * interaction_create_t&, and returns void.
   */
  event_router_t<interaction_create_t> on_interaction_create;

  /**
   * @brief Called when a button is clicked attached to a message.
   * Button clicks are triggered by discord when buttons are clicked which you
   * have associated with a message using dpp::component.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type button_click_t&,
   * and returns void.
   */
  event_router_t<button_click_t> on_button_click;

  /**
   * @brief Called when an auto completed field needs suggestions to present to
   * the user This is triggered by discord when option choices have auto
   * completion enabled which you have associated with a dpp::slashcommand.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type autocomplete_t&,
   * and returns void.
   */
   ...
   * Select menu clicks are triggered by discord when select menus are clicked
   * which you have associated with a message using dpp::component.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type select_click_t&,
   * and returns void.
   */
   ...
   * Form submits are triggered by discord when modal dialogs are submitted
   * which you have associated with a slash command using
   * dpp::interaction_modal_response.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type form_submit_t&, and
   * returns void.
   */
   ...
   * explicitly with dpp::guild_delete, or via the guild being unavailable due
   * to an outage.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type guild_delete_t&,
   * and returns void.
   */
   ...
   * The channel will still be temporarily available in the cache. Pointers to
   * the channel should not be retained long-term as they will be deleted by the
   * garbage collector.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type channel_delete_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type channel_update_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type ready_t&, and
   * returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type message_delete_t&,
   * and returns void.
   */
  event_router_t<message_delete_t> on_message_delete;

  /**
   * @brief Called when a user leaves a guild (either through being kicked, or
   * choosing to leave)
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_member_remove_t&, and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type resumed_t&, and
   * returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_role_create_t&, and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type typing_start_t&,
   * and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * message_reaction_add_t&, and returns void.
   */
  event_router_t<message_reaction_add_t> on_message_reaction_add;

  /**
   * @brief Called when a set of members is received for a guild.
   * D++ will request these for all new guilds if needed, after the
   * on_guild_create events.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_members_chunk_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * message_reaction_remove_t&, and returns void.
   */
  event_router_t<message_reaction_remove_t> on_message_reaction_remove;

  /**
   * @brief Called when a new guild is created.
   * D++ will request members for the guild for its cache using
   * guild_members_chunk.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type guild_create_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type channel_create_t&,
   * and returns void.
   */
  event_router_t<channel_create_t> on_channel_create;

  /**
   * @brief Called when all reactions for a particular emoji are removed from a
   * message.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * message_reaction_remove_emoji_t&, and returns void.
   */
  event_router_t<message_reaction_remove_emoji_t>
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * message_delete_bulk_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_role_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_role_delete_t&, and returns void.
   */
  event_router_t<guild_role_delete_t> on_guild_role_delete;

  /**
   * @brief Called when a message is pinned.
   * Note that the pinned message is not returned to this event, just the
   * timestamp of the last pinned message.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * channel_pins_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * message_reaction_remove_all_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_server_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_emojis_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_stickers_update_t&, and returns void.
   */
   ...
   * You will receive many of these, very often, and receiving them will
   * significantly increase your bot's CPU usage. If you don't need them it is
   * recommended to not ask for them.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type presence_update_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type webhooks_update_t&,
   * and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_member_add_t&, and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type invite_delete_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type guild_update_t&,
   * and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_integrations_update_t&, and returns void.
   */
  event_router_t<guild_integrations_update_t> on_guild_integrations_update;

  /**
   * @brief Called when details of a guild member (e.g. their roles or nickname)
   * are updated.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_member_update_t&, and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type invite_create_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type message_update_t&,
   * and returns void.
   */
   ...
   * This is separate to guild_member_update and includes things such as an
   * avatar change, username change, discriminator change or change in
   * subscription status for nitro.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type user_update_t&, and
   * returns void.
   */
   ...
   * Note that D++ does not cache messages. If you want to cache these objects
   * you should create something yourself within your bot. Caching of messages
   * is not on the roadmap to be supported as it consumes excessive amounts of
   * RAM.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type message_create_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type guild_ban_add_t&,
   * and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_ban_remove_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * integration_create_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * integration_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * integration_delete_t&, and returns void.
   */
  event_router_t<integration_delete_t> on_integration_delete;

  /**
   * @brief Called when a thread is created
   * Note: Threads are not cached by D++, but a list of thread IDs is accessible
   * in a guild object
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type thread_create_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type thread_update_t&,
   * and returns void.
   */
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type thread_delete_t&,
   * and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * thread_list_sync_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * thread_member_update_t&, and returns void.
   */
  event_router_t<thread_member_update_t> on_thread_member_update;

  /**
   * @brief Called when a thread's member list is updated (without GUILD_MEMBERS
   * intent, is only called for current user)
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * thread_members_update_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_scheduled_event_create_t&, and returns void.
   */
  event_router_t<guild_scheduled_event_create_t>
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_scheduled_event_update_t&, and returns void.
   */
  event_router_t<guild_scheduled_event_update_t>
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_scheduled_event_delete_t&, and returns void.
   */
  event_router_t<guild_scheduled_event_delete_t>
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_scheduled_event_user_add_t&, and returns void.
   */
  event_router_t<guild_scheduled_event_user_add_t>
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * guild_scheduled_event_user_remove_t&, and returns void.
   */
  event_router_t<guild_scheduled_event_user_remove_t>
	  on_guild_scheduled_event_user_remove;

  /**
   * @brief Called when packets are sent from the voice buffer.
   * The voice buffer contains packets that are already encoded with Opus and
   * encrypted with Sodium, and merged into packets by the repacketizer, which
   * is done in the dpp::discord_voice_client::send_audio method. You should use
   * the buffer size properties of dpp::voice_buffer_send_t to determine if you
   * should fill the buffer with more content.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_buffer_send_t&, and returns void.
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_user_talking_t&, and returns void.
   */
   ...
   * Note that this is not directly attached to the READY event of the
   * websocket, as there is further connection that needs to be done before
   * audio is ready to send.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type voice_ready_t&, and
   * returns void.
   */
   ...
   * their user id in the event, if a user can be attributed to the received
   * audio.
   *
   ...
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type voice_receive_t&,
   * and returns void.
   */
   ...
   * Track markers are arbitrarily placed "bookmarks" in the audio buffer,
   * placed by the bot developer. Each track marker can have a string value
   * associated with it which is specified in
   * dpp::discord_voice_client::insert_marker and returned to this event.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * voice_track_marker_t&, and returns void.
   */
   ...
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID.
   *
   */
   ...
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * stage_instance_update_t&, and returns void.
   */
  event_router_t<stage_instance_update_t> on_stage_instance_update;

  /**
   * @brief Called when an existing stage instance is deleted from a stage
   * channel.
   *
   * @note Use operator() to attach a lambda to this event, and the detach
   * method to detach the listener using the returned ID. The function signature
   * for this event takes a single `const` reference of type
   * stage_instance_delete_t&, and returns void.
   */
  event_router_t<stage_instance_delete_t> on_stage_instance_delete;

  /**
   * @brief Post a REST request. Where possible use a helper method instead like
   * message_create
   *
   ...
   * @param callback Function to call when the HTTP call completes. The callback
   * parameter will contain amongst other things, the decoded json.
   * @param filename Filename to post for POST requests (for uploading files)
   * @param filecontent File content to post for POST requests (for uploading
   * files)
   */
  void post_rest(const std::string &endpoint,
                 const std::string &major_parameters,
                 const std::string &parameters, http_method method,
                 const std::string &postdata, json_encode_t callback,
                 const std::string &filename = "",
                 const std::string &filecontent = "");

  /**
   * @brief Post a multipart REST request. Where possible use a helper method
   * instead like message_create
   *
   * @param endpoint Endpoint to post to, e.g. /api/guilds
   * @param major_parameters Major parameters for the endpoint e.g. a guild id
   * @param parameters Minor parameters for the API request
   ...
   * @param callback Function to call when the HTTP call completes. The callback
   * parameter will contain amongst other things, the decoded json.
   * @param filename List of filenames to post for POST requests (for uploading
   * files)
   * @param filecontent List of file content to post for POST requests (for
   * uploading files)
   */
  void post_rest_multipart(const std::string &endpoint,
                           const std::string &major_parameters,
                           const std::string &parameters, http_method method,
                           const std::string &postdata, json_encode_t callback,
  ...
  /**
   * @brief Make a HTTP(S) request. For use when wanting asynchronous access to
   * HTTP APIs outside of Discord.
   *
   * @param url Full URL to post to, e.g. https://api.somewhere.com/v1/foo/
   * @param method Method, e.g. GET, POST
   * @param callback Function to call when the HTTP call completes. No
   * processing is done on the returned data.
   * @param postdata POST data
   * @param mimetype MIME type of POST data
   * @param headers Headers to send with the request
   */
  void request(const std::string &url, http_method method,
               http_completion_event callback, const std::string &postdata = "",
   ...
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void interaction_response_create(
	  snowflake interaction_id, const std::string &token,
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void interaction_response_edit(
	  const std::string &token, const message &m,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Create a global slash command (a bot can have a maximum of 100 of
   * these).
   *
   * @note Global commands are cached by discord server-side and can take up to
   * an hour to be visible. For testing, you should use
   * cluster::guild_command_create instead.
   *
   * @see
   * https://discord.com/developers/docs/interactions/application-commands#create-global-application-command
   * @param s Slash command to create
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::slashcommand object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void global_command_create(
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void global_command_get(
	  snowflake id, command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::auditlog object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_auditlog_get(snowflake guild_id,
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_command_create(
	  const slashcommand &s, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Create/overwrite guild slash commands.
   * Any existing guild slash commands on this guild will be deleted and
   * replaced with these.
   *
   * @see
   * https://discord.com/developers/docs/interactions/application-commands#bulk-overwrite-guild-application-commands
   * @param commands Vector of slash commands to create/update.
   * New guild commands will be available in the guild immediately. If the
   * command did not already exist, it will count toward daily application
   * command create limits.
   * @param guild_id Guild ID to create/update the slash commands in
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a list of dpp::slashcommand object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_bulk_command_create(
   ...
   * @note Global commands are cached by discord server-side and can take up to
   * an hour to be visible. For testing, you should use
   * cluster::guild_bulk_command_create instead.
   *
   * @see
   * https://discord.com/developers/docs/interactions/application-commands#bulk-overwrite-global-application-commands
   * @param commands Vector of slash commands to create/update.
   * overwriting existing commands that are registered globally for this
   * application. Updates will be available in all guilds after 1 hour. Commands
   * that do not already exist will count toward daily application command
   * create limits.
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a list of dpp::slashcommand object in
   * confirmation_callback_t::value. On failure, the value is undefined and
  ...
  /**
   * @brief Edit a global slash command (a bot can have a maximum of 100 of
   * these)
   *
   * @see
   ...
   * @note The returned slash commands will not have permissions set, you need
   * to use a permissions getter e.g. dpp::guild_commands_get_permissions to get
   * the guild command permissions
   * @param id The ID of the slash command
   ...
   */
  void
  guild_command_get(snowflake id, snowflake guild_id,
                    command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_command_edit_permissions(
	  const slashcommand &s, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_command_permissions
   * object in confirmation_callback_t::value. On failure, the value is
   * undefined and confirmation_callback_t::is_error() method will return true.
   * You can obtain full error details with
   * confirmation_callback_t::get_error().
   */
  void guild_command_get_permissions(
	  snowflake id, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit/Overwrite the permissions of all existing slash commands in a
   * guild
   *
   * @note You can only add up to 10 permission overwrites for a command
   *
   ...
   * @warning The endpoint will overwrite all existing permissions for all
   * commands of the application in a guild, including slash commands, user
   * commands, and message commands. Meaning that if you forgot to pass a slash
   * command, the permissions of it might be removed.
   * @param commands A vector of slash commands to edit/overwrite the
   * permissions for
   * @param guild_id Guild ID to edit permissions of the slash commands in
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_command_permissions_map
   * object in confirmation_callback_t::value. On failure, the value is
   * undefined and confirmation_callback_t::is_error() method will return true.
   * You can obtain full error details with
   * confirmation_callback_t::get_error().
   */
  void guild_bulk_command_edit_permissions(
	  const std::vector<slashcommand> &commands, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Delete a global slash command (a bot can have a maximum of 100 of
   * these)
   *
   * @see
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::slashcommand_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_commands_get(snowflake guild_id,
                          command_completion_event_t callback);
   ...
   * confirmation_callback_t::get_error().
   */
  void guild_commands_get_permissions(snowflake guild_id,
                                      command_completion_event_t callback);
  ...
  /**
   * @brief Create a direct message, also create the channel for the direct
   * message if needed
   *
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::message object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void direct_message_create(
	  snowflake user_id, const message &m,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_get(snowflake message_id, snowflake channel_id,
                   command_completion_event_t callback);

  /**
   * @brief Get multiple messages.
   *
   * This function will attempt to fetch as many messages as possible using
   * multiple API calls if needed.
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#get-channel-messages
   * @param channel_id Channel ID to retrieve messages for
   * @param around Messages should be retrieved around this ID if this is set to
   * non-zero
   * @param before Messages before this ID should be retrieved if this is set to
   * non-zero
   * @param after Messages after this ID should be retrieved if this is set to
   * non-zero
   * @param limit This number of messages maximum should be returned, up to a
   * maximum of 100.
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::message_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void messages_get(snowflake channel_id, snowflake around, snowflake before,
                    snowflake after, uint64_t limit,
                    command_completion_event_t callback);

  /**
   * @brief Send a message to a channel. The callback function is called when
   ...
   */
  void
  message_create(const struct message &m,
                 command_completion_event_t callback = utility::log_error());

  /**
   * @brief Crosspost a message. The callback function is called when the
   ...
   */
  void
  message_crosspost(snowflake message_id, snowflake channel_id,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit a message on a channel. The callback function is called when
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_edit(const struct message &m,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Add a reaction to a message. The reaction string must be either an
   * `emojiname:id` or a unicode character.
   *
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_add_reaction(
	  const struct message &m, const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Delete own reaction from a message. The reaction string must be
   * either an `emojiname:id` or a unicode character.
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#delete-own-reaction
   * @param m Message to delete own reaction from
   * @param reaction Reaction to delete. The reaction should be in the form
   * emojiname:id
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_delete_own_reaction(
	  const struct message &m, const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Delete a user's reaction from a message. The reaction string must be
   * either an `emojiname:id` or a unicode character
   *
   * @see
   ...
   * @param reaction Reaction to remove. Reactions should be in the form
   * emojiname:id
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_delete_reaction(
	  const struct message &m, snowflake user_id, const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get reactions on a message for a particular emoji. The reaction
   * string must be either an `emojiname:id` or a unicode character
   *
   * @see https://discord.com/developers/docs/resources/channel#get-reactions
   * @param m Message to get reactions for
   * @param reaction Reaction should be in the form emojiname:id or a unicode
   * character
   * @param before Reactions before this ID should be retrieved if this is set
   * to non-zero
   * @param after Reactions before this ID should be retrieved if this is set to
   * non-zero
   * @param limit This number of reactions maximum should be returned
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::user_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void message_get_reactions(const struct message &m,
                             const std::string &reaction, snowflake before,
                             snowflake after, snowflake limit,
                             command_completion_event_t callback);
  ...
  /**
   * @brief Delete all reactions on a message using a particular emoji. The
   * reaction string must be either an `emojiname:id` or a unicode character
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#delete-all-reactions-for-emoji
   * @param m Message to delete reactions from
   * @param reaction Reaction to delete, in the form emojiname:id or a unicode
   * character
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_delete_reaction_emoji(
	  const struct message &m, const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Add a reaction to a message by id. The reaction string must be
   * either an `emojiname:id` or a unicode character.
   *
   * @see
   ...
   */
  void message_add_reaction(
	  snowflake message_id, snowflake channel_id, const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Delete own reaction from a message by id. The reaction string must
   * be either an `emojiname:id` or a unicode character.
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#delete-own-reaction
  ...
  /**
   * @brief Delete a user's reaction from a message by id. The reaction string
   * must be either an `emojiname:id` or a unicode character
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#delete-user-reaction
   * @param message_id Message to delete reactions from
   ...
   */
  void message_delete_reaction(
	  snowflake message_id, snowflake channel_id, snowflake user_id,
	  const std::string &reaction,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get reactions on a message for a particular emoji by id. The
   * reaction string must be either an `emojiname:id` or a unicode character
   *
   * @see https://discord.com/developers/docs/resources/channel#get-reactions
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_get_reactions(snowflake message_id, snowflake channel_id,
                             const std::string &reaction, snowflake before,
                             snowflake after, snowflake limit,
                             command_completion_event_t callback);
  ...
  /**
   * @brief Delete all reactions on a message using a particular emoji by id.
   ...
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   *
   ...
   */
  void
  message_delete(snowflake message_id, snowflake channel_id,
                 command_completion_event_t callback = utility::log_error());
   ...
   * cluster::set_audit_reason() method.
   *
   * @note If any message provided older than 2 weeks or any duplicate message
   * ID, it will fail.
   *
   * @see
   * https://discord.com/developers/docs/resources/channel#bulk-delete-messages
   * @param message_ids List of message IDs to delete (at least 2 and at most
   * 100 message IDs)
   * @param channel_id Channel to delete from
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_delete_bulk(
	  const std::vector<snowflake> &message_ids, snowflake channel_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::channel object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::channel_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   */
  void
  channel_create(const class channel &c,
                 command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_edit(const class channel &c,
                    command_completion_event_t callback = utility::log_error());
   ...
   * Requires `MANAGE_CHANNELS` permission. Fires multiple `Channel Update
   * Gateway` events. Only channels to be modified are required.
   *
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   * @see
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_edit_positions(
	  const std::vector<channel> &c,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param member true if the overwrite_id is a user id, false if it is a
   * channel id
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_edit_permissions(
	  const class channel &c, const snowflake overwrite_id,
	  const uint32_t allow, const uint32_t deny, const bool member,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void channel_edit_permissions(
	  const snowflake channel_id, const snowflake overwrite_id,
	  const uint32_t allow, const uint32_t deny, const bool member,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  channel_delete(snowflake channel_id,
                 command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::invite object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void invite_get(const std::string &invite,
                  command_completion_event_t callback);
   ...
   */
  void
  invite_delete(const std::string &invite,
                command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::invite_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_invites_get(const class channel &c,
                           command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_invite_create(
	  const class channel &c, const class invite &i,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_pins_get(snowflake channel_id,
                        command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void gdm_add(snowflake channel_id, snowflake user_id,
               const std::string &access_token, const std::string &nick,
               command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void gdm_remove(snowflake channel_id, snowflake user_id,
                  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_delete_permission(
	  const class channel &c, snowflake overwrite_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void channel_follow_news(
	  const class channel &c, snowflake target_channel_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  channel_typing(const class channel &c,
                 command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void message_pin(snowflake channel_id, snowflake message_id,
                   command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  message_unpin(snowflake channel_id, snowflake message_id,
                command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get a guild
   *
   * Returns the guild object for the given id. This endpoint will also return
   * approximate_member_count and approximate_presence_count for the guild.
   * @see https://discord.com/developers/docs/resources/guild#get-guild
   * @param g Guild ID to retrieve
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  ...
  /**
   * @brief Get a guild preview. Returns a guild object but only a subset of the
   * fields will be populated.
   *
   * Returns the guild preview object for the given id `g`. If the user is not
   * in the guild, then the guild must be lurkable (it must be Discoverable or
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_member object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_member(snowflake guild_id, snowflake user_id,
                        command_completion_event_t callback);

  /**
   * @brief Search for guild members based on whether their username or nickname
   * starts with the given string.
   *
   * @note This endpoint is restricted according to whether the `GUILD_MEMBERS`
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_member_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_search_members(snowflake guild_id, const std::string &query,
                            uint16_t limit,
                            command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_members(snowflake guild_id, uint16_t limit, snowflake after,
                         command_completion_event_t callback);

  /**
   * @brief Add guild member. Needs a specific oauth2 scope, from which you get
   * the access_token.
   *
   * Adds a user to the guild, provided you have a valid oauth2 access token for
   * the user with the guilds.join scope. Returns the guild_member, which is
   * defaulted if the user is already a member of the guild. Fires a `Guild
   * Member Add` Gateway event.
   *
   ...
   * The bot must be a member of the guild with `CREATE_INSTANT_INVITE`
   * permission.
   ...
   */
  void
  guild_add_member(const guild_member &gm, const std::string &access_token,
                   command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit the properties of an existing guild member
   *
   * Modify attributes of a guild member. Returns the guild_member. Fires a
   * `Guild Member Update` Gateway event. If the `channel_id` is set to 0, this
   * will force the target user to be disconnected from voice. To remove a
   * timeout, set the `communication_disabled_until` to a non-zero time in the
   * past, e.g. 1. When moving members to channels, the API user must have
   * permissions to both connect to the channel and have the `MOVE_MEMBERS`
   * permission.
   * @see
   ...
   */
  void
  guild_edit_member(const guild_member &gm,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Moves the guild member to a other voice channel, if member is
   * connected to one
   *
   * Fires a `Guild Member Update` Gateway event.
   * @note When moving members to channels, the API user __must__ have
   * permissions to both connect to the channel and have the `MOVE_MEMBERS`
   * permission.
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   ...
   */
  void
  guild_member_move(const snowflake channel_id, const snowflake guild_id,
   ...
   * Fires a `Guild Member Update` Gateway event.
   *
   * @deprecated Deprecated in favor of Modify Current Member. Will be replaced
   * by dpp::cluster::guild_current_member_edit
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   * @see
   ...
   */
  void
  guild_member_kick(snowflake guild_id, snowflake user_id,
                    command_completion_event_t callback = utility::log_error());
   ...
   * @param communication_disabled_until The timestamp when the user's timeout
   * will expire (up to 28 days in the future). Set to 0 to remove the timeout
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::confirmation object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   ...
   * banned user. Requires the `BAN_MEMBERS` permission. Fires a `Guild Ban Add`
   * Gateway event.
   ...
   * @param delete_message_days How many days of their user's messages to also
   * delete
   * @param reason Reason for ban
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::ban object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void
  guild_ban_add(snowflake guild_id, snowflake user_id,
                uint32_t delete_message_days, const std::string &reason,
                command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  guild_ban_delete(snowflake guild_id, snowflake user_id,
                   command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::ban_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_ban(snowflake guild_id, snowflake user_id,
                     command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::dtemplate object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void template_get(const std::string &code,
                    command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::dtemplate_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_templates_get(snowflake guild_id,
                           command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_template_create(snowflake guild_id, const std::string &name,
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_template_sync(
	  snowflake guild_id, const std::string &code,
	  command_completion_event_t callback = utility::log_error());
  ...
  void guild_template_modify(
	  snowflake guild_id, const std::string &code, const std::string &name,
	  const std::string &description,
	  command_completion_event_t callback = utility::log_error());
   ...
   * bootstrap a guild with additional roles, keep this in mind. The required id
   * field within each role object is an integer placeholder, and will be
   * replaced by the API upon consumption. Its purpose is to allow you to
   * overwrite a role's permissions in a channel when also passing in channels
   * with the channels array. When using the channels parameter, the position
   * field is ignored, and none of the default channels are created. The id
   * field within each channel object may be set to an integer placeholder, and
   * will be replaced by the API upon consumption. Its purpose is to allow you
   * to create `GUILD_CATEGORY` channels by setting the `parent_id` field on any
   * children to the category's id field. Category channels must be listed
   * before any children.
   *
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_create(const class guild &g,
                    command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_edit(const class guild &g,
                  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_delete(snowflake guild_id,
                    command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::emoji_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_emojis_get(snowflake guild_id,
                        command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::emoji object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_emoji_get(snowflake guild_id, snowflake emoji_id,
                       command_completion_event_t callback);

  /**
   * @brief Create single emoji.
   * You must ensure that the emoji passed contained image data using the
   * emoji::load_image() method.
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   *
  ...
  void guild_emoji_create(
	  snowflake guild_id, const class emoji &newemoji,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  guild_emoji_edit(snowflake guild_id, const class emoji &newemoji,
                   command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_emoji_delete(
	  snowflake guild_id, snowflake emoji_id,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get prune counts
   *
   * Returns a prune object indicating the number of members that would be
   * removed in a prune operation. Requires the `KICK_MEMBERS` permission. By
   * default, prune will not remove users with roles. You can optionally include
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::prune object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_prune_counts(snowflake guild_id, const struct prune &pruneinfo,
                              command_completion_event_t callback);

  /**
   * @brief Begin guild prune
   *
   * Begin a prune operation. Requires the `KICK_MEMBERS` permission. Returns a
   * prune object indicating the number of members that were removed in the
   * prune operation. For large guilds it's recommended to set the
   * `compute_prune_count` option to false, forcing 'pruned' to 0. Fires
   * multiple `Guild Member Remove` Gateway events. By default, prune will not
   * remove users with roles. You can optionally include specific roles in your
   * prune by providing the `include_roles` parameter. Any inactive user that
   * has a subset of the provided role(s) will be included in the prune and
   * users with additional roles will not.
   *
   ...
   */
  void
  guild_begin_prune(snowflake guild_id, const struct prune &pruneinfo,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get guild voice regions.
   *
   * Voice regions per guild are somewhat deprecated in preference of
   * per-channel voice regions. Returns a list of voice region objects for the
   * guild. Unlike the similar /voice route, this returns VIP servers when the
   * guild is VIP-enabled.
   *
   * @see
   * https://discord.com/developers/docs/resources/guild#get-guild-voice-regions
   * @param guild_id Guild ID to get voice regions for
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::voiceregion_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_voice_regions(snowflake guild_id,
                               command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_invites(snowflake guild_id,
                         command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::integration_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_integrations(snowflake guild_id,
                              command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::integration object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_modify_integration(
	  snowflake guild_id, const class integration &i,
	  command_completion_event_t callback = utility::log_error());
   ...
   * the `MANAGE_GUILD` permission. Fires a Guild Integrations Update Gateway
   * event.
   *
   * @see
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_delete_integration(
	  snowflake guild_id, snowflake integration_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_widget object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_get_widget(snowflake guild_id,
                        command_completion_event_t callback);
   ...
   */
  void
  guild_edit_widget(snowflake guild_id, const class guild_widget &gw,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Get guild vanity url, if enabled
   *
   * Returns a partial dpp::invite object for guilds with that feature enabled.
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::invite object in
   * confirmation_callback_t::value filled to match the vanity url. On failure,
   * the value is undefined and confirmation_callback_t::is_error() method will
   * return true. You can obtain full error details with
   * confirmation_callback_t::get_error().
   */
  void guild_get_vanity(snowflake guild_id,
                        command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::webhook object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void
  create_webhook(const class webhook &w,
                 command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::webhook_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void get_guild_webhooks(snowflake guild_id,
                          command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void get_channel_webhooks(snowflake channel_id,
                            command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void get_webhook_with_token(snowflake webhook_id, const std::string &token,
                              command_completion_event_t callback);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void edit_webhook(const class webhook &wh,
                    command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit webhook with token (token is encapsulated in the webhook
   * object)
   * @see
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void edit_webhook_with_token(
	  const class webhook &wh,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  delete_webhook(snowflake webhook_id,
                 command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void delete_webhook_with_token(
	  snowflake webhook_id, const std::string &token,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param thread_id Send a message to the specified thread within a webhook's
   ...
   */
  void
  execute_webhook(const class webhook &wh, const struct message &m,
                  bool wait = false, snowflake thread_id = 0,
                  command_completion_event_t callback = utility::log_error());
   ...
   * @note the attachments array must contain all attachments that should be
   * present after edit, including retained and new attachments provided in the
   * request body.
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void edit_webhook_message(
	  const class webhook &wh, const struct message &m,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void delete_webhook_message(
	  const class webhook &wh, snowflake message_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::role_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::role object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void role_create(const class role &r,
                   command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit a role on a guild
   *
   * Requires the `MANAGE_ROLES` permission. Returns the updated role on
   * success. Fires a `Guild Role Update` Gateway event.
   *
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void role_edit(const class role &r,
                 command_completion_event_t callback = utility::log_error());

  /**
   * @brief Edit multiple role's position in a guild. Returns a list of all
   * roles of the guild on success.
   *
   * Modify the positions of a set of role objects for the guild. Requires the
   * `MANAGE_ROLES` permission. Fires multiple `Guild Role Update` Gateway
   * events.
   *
   * @see
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void roles_edit_position(
	  snowflake guild_id, const std::vector<role> &roles,
	  command_completion_event_t callback = utility::log_error());

  /**
   * @brief Delete a role
   *
   * Requires the `MANAGE_ROLES` permission. Fires a `Guild Role Delete` Gateway
   * event.
   *
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void role_delete(snowflake guild_id, snowflake role_id,
                   command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::user_identified object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   * @note The user_identified object is a subclass of dpp::user which contains
   * further details if you have the oauth2 identify or email scopes. If you do
   * not have these scopes, these fields are empty. You can safely convert a
   * user_identified to user with `dynamic_cast`.
   * @note unless you want something special from `dpp::user_identified` or
   * you've turned off caching, you have no need to call this. Call
   * `dpp::find_user` instead that looks up the user in the cache rather than a
   * REST call.
   */
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::application object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_current_member_edit(
	  snowflake guild_id, const std::string &nickname,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::connection_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::guild_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   *
   * Modifies the current member in a guild. Returns the updated guild_member
   * object on success. Fires a `Guild Member Update` Gateway event.
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::user object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   * @throw dpp::exception Image data is larger than the maximum size of 256
   * kilobytes
   */
  void
  current_user_edit(const std::string &nickname,
   ...
   */
  void
  create_dm_channel(snowflake user_id,
                    command_completion_event_t callback = utility::log_error());
   ...
   * @param auto_archive_duration Duration after which thread auto-archives. Can
   * be set to - 60, 1440 (for boosted guilds can also be: 4320, 10080)
   * @param thread_type Type of thread - GUILD_PUBLIC_THREAD, GUILD_NEWS_THREAD,
   * GUILD_PRIVATE_THREAD
   * @param invitable whether non-moderators can add other non-moderators to a
   * thread; only available when creating a private thread
   * @param rate_limit_per_user amount of seconds a user has to wait before
   * sending another message (0-21600); bots, as well as users with the
   * permission manage_messages, manage_thread, or manage_channel, are
   * unaffected
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::thread object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void
  thread_create(const std::string &thread_name, snowflake channel_id,
                uint16_t auto_archive_duration, channel_type thread_type,
                bool invitable, uint16_t rate_limit_per_user,
                command_completion_event_t callback = utility::log_error());

  /**
   * @brief Create a thread with a message (Discord: ID of a thread is same as
   * message ID)
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void thread_create_with_message(
	  const std::string &thread_name, snowflake channel_id,
	  snowflake message_id, uint16_t auto_archive_duration,
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void current_user_leave_thread(
	  snowflake thread_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  thread_member_add(snowflake thread_id, snowflake user_id,
                    command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void thread_member_remove(
	  snowflake thread_id, snowflake user_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes
   * On success the callback will contain a dpp::thread_member object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void thread_member_get(const snowflake thread_id, const snowflake user_id,
                         command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes
   * On success the callback will contain a dpp::thread_member_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void thread_members_get(snowflake thread_id,
                          command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes
   * On success the callback will contain a dpp::thread_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void threads_get_active(snowflake channel_id,
                          command_completion_event_t callback);

  /**
   * @brief Get public archived threads in a channel (Sorted by
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void threads_get_public_archived(snowflake channel_id,
                                   time_t before_timestamp, uint16_t limit,
                                   command_completion_event_t callback);

  /**
   * @brief Get private archived threads in a channel (Sorted by
   * archive_timestamp in descending order)
   * @see https://discord.com/developers/docs/topics/thread
   * @param channel_id Channel to get public archived threads for
   * @param before_timestamp Get threads before this timestamp
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void threads_get_private_archived(snowflake channel_id,
                                    time_t before_timestamp, uint16_t limit,
                                    command_completion_event_t callback);

  /**
   * @brief Get private archived threads in a channel which current user has
   joined (Sorted by ID in descending order)
   ...
   * On success the callback will contain a dpp::thread_map object in
   ...
   full error details with confirmation_callback_t::get_error().
   */
  void threads_get_joined_private_archived(snowflake channel_id,
                                           snowflake before_id, uint16_t limit,
                                           command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::sticker object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_sticker_create(
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_sticker_delete(
	  snowflake sticker_id, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_sticker_get(snowflake id, snowflake guild_id,
                         command_completion_event_t callback);
   ...
   * @param guild_id Guild ID of the guild where the sticker is
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::sticker_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_stickers_get(snowflake guild_id,
                          command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::sticker_pack_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * On success the callback will contain a dpp::stage_instance object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   * @note This method supports audit log reasons set by the
   * cluster::set_audit_reason() method.
   */
  void stage_instance_create(
	  const stage_instance &instance,
	  command_completion_event_t callback = utility::log_error());
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void stage_instance_get(const snowflake channel_id,
                          command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::gateway object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::scheduled_event_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_events_get(snowflake guild_id,
                        command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::event_member_map object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_event_users_get(snowflake guild_id, snowflake event_id,
                             command_completion_event_t callback,
                             uint8_t limit = 100, snowflake before = 0,
                             snowflake after = 0);
   ...
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_event_delete(
	  snowflake event_id, snowflake guild_id,
	  command_completion_event_t callback = utility::log_error());
   ...
   */
  void
  guild_event_edit(const scheduled_event &event,
                   command_completion_event_t callback = utility::log_error());
   ...
   * @param callback Function to call when the API call completes.
   * On success the callback will contain a dpp::scheduled_event object in
   * confirmation_callback_t::value. On failure, the value is undefined and
   * confirmation_callback_t::is_error() method will return true. You can obtain
   * full error details with confirmation_callback_t::get_error().
   */
  void guild_event_get(snowflake guild_id, snowflake event_id,
                       command_completion_event_t callback);
   ...
   * @param callback Function to call when the API call completes.
   * @param suppress True if the user's audio should be suppressed, false if it
   * should not
   * @param request_to_speak_timestamp The time at which we requested to speak,
   * or 0 to clear the request. The time set here must be the current time or in
   * the future. On success the callback will contain a dpp::scheduled_event
   * object in confirmation_callback_t::value. On failure, the value is
   * undefined and confirmation_callback_t::is_error() method will return true.
   * You can obtain full error details with
   * confirmation_callback_t::get_error().
   * @throw std::logic_exception You attempted to set a
   * request_to_speak_timestamp in the past which is not the value of 0.
   */
  void current_user_set_voice_state(
	  snowflake guild_id, snowflake channel_id, bool suppress = false,
   ...
   * - You must have the `MUTE_MEMBERS` permission. (Since suppression is the
   * only thing that is available currently)
   ...
   * @param callback Function to call when the API call completes.
   * @param suppress True if the user's audio should be suppressed, false if it
   * should not On success the callback will contain a dpp::scheduled_event
   * object in confirmation_callback_t::value. On failure, the value is
   * undefined and confirmation_callback_t::is_error() method will return true.
   * You can obtain full error details with
   * confirmation_callback_t::get_error().
   */
  void user_set_voice_state(
	  snowflake user_id, snowflake guild_id, snowflake channel_id,
	  bool suppress = false,
	  command_completion_event_t callback = utility::log_error());
...
/**
 * @brief A timed_listener is a way to temporarily attach to an event for a
 * specific timeframe, then detach when complete. A lambda may also be
 * optionally called when the timeout is reached. Destructing the timed_listener
 * detaches any attached event listeners, and cancels any created timers, but
 * does not call any timeout lambda.
 *
 * @tparam attached_event Event within cluster to attach to within the
 * cluster::dispatch member (dpp::dispatcher object)
 * @tparam listening_function Definition of lambda function that matches up with
 * the attached_event.
 */
...
public:
   ...
   * @param on_end An optional void() lambda to trigger when the timed_listener
   * times out. Calling the destructor before the timeout is reached does not
   * call this lambda.
   * @param listener Lambda to receive events. Type must match up properly with
   * that passed into the 'event' parameter.
   */
  timed_listener(cluster *cl, uint64_t _duration, attached_event &event,
                 listening_function listener, timer_callback_t on_end = {})
 ...
 *
 * This template must be specialised. There are premade specialisations which
 * you can use such as dpp::reaction_collector and dpp::message_collector. For
 * these specialised instances all you need to do is derive a simple class from
 * them which implements collector::completed().
 *
 * A collector will run for the specified number of seconds, attaching itself to
 * the given event. During this time any events pass through the collector and
 * collector::filter(). This function can return a pointer to an object to allow
 * a copy of that object to be stored to a vector, or it can return nullptr to
 * do nothing with that object. For example a collector attached to
 * on_message_create would receive an event with the type message_create_t, and
 * from this may decide to extract the message_create_t::msg structure,
 * returning a pointer to it, or instead may choose to return a nullptr.
 *
 * When either the predetermined timeout is reached, or the collector::cancel()
 * method is called, or the collector is destroyed, the collector::completed()
 * method is called, which will be passed a list of collected objects in the
...
public:
   ...
   *
   * The timer for the collector begins immediately on construction of the
   * object.
   *
  ...
  collector(class cluster *cl, uint64_t duration, event_router_t<T> &event)
	...
	tl = new dpp::timed_listener<event_router_t<T>,
	                             std::function<void(const T &)>>(
		cl, duration, event, f, [this](dpp::timer timer_handle) {
		  if (!triggered) {
			triggered = true;
			completed(stored);
		  }
   ...
   * object to be stored.
   *
   * Here is an example of how to filter messages which have specific text in
   *them. This should be used with the specialised type dpp::message_collector
   *
   * ```cpp
   ...
   *
   * Use this if you have met the conditions for which you are collecting
   * objects early, e.g. you were watching for a message containing 'yes' or
   * 'no' and have received it before the time is up.
   *
   * @note Causes calling of the completed() method if it has not yet been
   * called.
   */
  virtual void cancel() {
	delete tl;
