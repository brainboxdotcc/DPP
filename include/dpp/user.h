...
enum user_flags : uint32_t
{
  ...
  /// User is a bot using HTTP interactions (shows online even when not
  /// connected to a websocket)
  u_bot_http_interactions = 0b00100000000000000000000,
};

/**
 * @brief Represents a user on discord. May or may not be a member of a
 * dpp::guild.
 */
...
public:
   ...
   *
   * @note To print the discriminator with leading zeroes, use something like
   * `fmt::format("{:04d}", discriminator)`
   */
   ...
   * returned.
   * @return std::string avatar url. If the user doesn't have an avatar, the
   * default user avatar url is returned
   */
   ...
   */
  [[nodiscard]] bool is_mfa_enabled() const;
  /**
   ...
   */
  [[nodiscard]] bool has_nitro_full() const;
  /**
   ...
   */
  [[nodiscard]] std::string format_username() const;
};

/**
 * @brief A user with additional fields only available via the oauth2 identify
 * scope. These are not included in dpp::user as additional scopes are needed to
 * fetch them which bots do not normally have.
 */
class DPP_EXPORT user_identified : public user,
								   public json_interface<user_identified>
{
public:
  ...
  utility::iconhash
	  banner; //!< Optional: the user's banner hash    identify (may be empty)
  uint32_t accent_color; //!< Optional: the user's banner color encoded as an
                         //!< integer representation of hexadecimal color code
                         //!< identify (may be empty)
  bool verified; //!< Optional: whether the email on this account has been
                 //!< verified       email
   ...
   */
  ~user_identified() override;

  /**
   * @brief Get the user identified's banner url if they have one, otherwise
   ...
   */
  [[nodiscard]] std::string get_banner_url(uint16_t size = 0) const;
};
...
/** A group of users */
using user_map = std::unordered_map<snowflake, user>;
