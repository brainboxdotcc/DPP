 ...
 *
 * @note Global commands are cached by discord server-side and can take up to an
 * hour to be visible. For testing, you should use
 ...
 * that do not already exist will count toward daily application command create
 * limits.
 * @return slashcommand_map returned object on completion
 * \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
 */
slashcommand_map
global_bulk_command_create_sync(const std::vector<slashcommand> &commands);
 ...
 *
 * @note Global commands are cached by discord server-side and can take up to an
 * hour to be visible. For testing, you should use cluster::guild_command_create
 * instead.
 *
 ...
 * Any existing guild slash commands on this guild will be deleted and replaced
 * with these.
 *
 ...
 * @param commands Vector of slash commands to create/update.
 * New guild commands will be available in the guild immediately. If the command
 * did not already exist, it will count toward daily application command create
 * limits.
 ...
 */
slashcommand_map
guild_bulk_command_create_sync(const std::vector<slashcommand> &commands,
                               snowflake guild_id);
...
guild_commands_get_permissions_sync(snowflake guild_id);
 ...
 * @param commands A vector of slash commands to edit/overwrite the permissions
 * for
 ...
 * an event handler.
 */
guild_command_permissions_map guild_bulk_command_edit_permissions_sync(
 ...
 * @note The returned slash commands will not have permissions set, you need to
 * use a permissions getter e.g. dpp::guild_commands_get_permissions to get the
...
guild_command_get_permissions_sync(snowflake id, snowflake guild_id);
 ...
 * an event handler.
 */
confirmation gdm_add_sync(snowflake channel_id, snowflake user_id,
 ...
 * an event handler.
 */
confirmation guild_ban_add_sync(snowflake guild_id, snowflake user_id,
 ...
 * field within each role object is an integer placeholder, and will be replaced
 * by the API upon consumption. Its purpose is to allow you to overwrite a
 * role's permissions in a channel when also passing in channels with the
 * channels array. When using the channels parameter, the position field is
 * ignored, and none of the default channels are created. The id field within
 * each channel object may be set to an integer placeholder, and will be
 * replaced by the API upon consumption. Its purpose is to allow you to create
 * `GUILD_CATEGORY` channels by setting the `parent_id` field on any children to
 * the category's id field. Category channels must be listed before any
 * children.
 *
 ...
 * `MANAGE_GUILD` permission. Fires a Guild Integrations Update Gateway event.
 *
 ...
 *
 * Returns a prune object indicating the number of members that would be removed
 * in a prune operation. Requires the `KICK_MEMBERS` permission. By default,
 * prune will not remove users with roles. You can optionally include specific
 * roles in your prune by providing the include_roles parameter. Any inactive
 * user that has a subset of the provided role(s) will be counted in the prune
 * and users with additional roles will not.
 *
 ...
 * an event handler.
 */
prune guild_get_prune_counts_sync(snowflake guild_id,
 ...
 * prune object indicating the number of members that were removed in the prune
 * operation. For large guilds it's recommended to set the `compute_prune_count`
 * option to false, forcing 'pruned' to 0. Fires multiple `Guild Member Remove`
 * Gateway events. By default, prune will not remove users with roles. You can
 * optionally include specific roles in your prune by providing the
 * `include_roles` parameter. Any inactive user that has a subset of the
 * provided role(s) will be included in the prune and users with additional
 * roles will not.
 *
 ...
 * Fires a `Guild Member Update` Gateway event.
 *
 * @deprecated Deprecated in favor of Modify Current Member. Will be replaced by
 ...
 * defaulted if the user is already a member of the guild. Fires a `Guild Member
 * Add` Gateway event.
 *
 ...
 *
 * Modify attributes of a guild member. Returns the guild_member. Fires a `Guild
 * Member Update` Gateway event. If the `channel_id` is set to 0, this will
 * force the target user to be disconnected from voice. To remove a timeout, set
 * the `communication_disabled_until` to a non-zero time in the past, e.g. 1.
 * When moving members to channels, the API user must have permissions to both
 ...
 * an event handler.
 */
guild_member_map guild_get_members_sync(snowflake guild_id, uint16_t limit,
                                        snowflake after);
 ...
 * an event handler.
 */
confirmation guild_member_add_role_sync(snowflake guild_id, snowflake user_id,
                                        snowflake role_id);
 ...
 * an event handler.
 */
confirmation guild_member_timeout_sync(snowflake guild_id, snowflake user_id,
 ...
 *
 * Fires a `Guild Member Update` Gateway event.
 * @note When moving members to channels, the API user __must__ have permissions
...
/**
 * @brief Send a message to a channel. The callback function is called when the
...
/**
 * @brief Crosspost a message. The callback function is called when the message
 * has been sent
 *
 ...
 * cluster::set_audit_reason() method.
 *
 * @note If any message provided older than 2 weeks or any duplicate message ID,
 * it will fail.
 *
 * @see dpp::cluster::message_delete_bulk
 * @see
 * https://discord.com/developers/docs/resources/channel#bulk-delete-messages
 * @param message_ids List of message IDs to delete (at least 2 and at most 100
 * message IDs)
...
/**
 * @brief Delete a message from a channel. The callback function is called when
...
/**
 * @brief Edit a message on a channel. The callback function is called when the
 ...
 * an event handler.
 */
message_map messages_get_sync(snowflake channel_id, snowflake around,
 ...
 *
 * Requires the `MANAGE_ROLES` permission. Returns the updated role on success.
...
/**
 * @brief Edit multiple role's position in a guild. Returns a list of all roles
 * of the guild on success.
 *
 * Modify the positions of a set of role objects for the guild. Requires the
 * `MANAGE_ROLES` permission. Fires multiple `Guild Role Update` Gateway events.
 *
 ...
 * an event handler.
 */
role_map roles_edit_position_sync(snowflake guild_id,
 ...
 * @return confirmation returned object on completion
 * @note This method supports audit log reasons set by the
 * cluster::set_audit_reason() method. \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
...
/**
 * @brief Get private archived threads in a channel (Sorted by archive_timestamp
 * in descending order)
...
/**
 * @brief Get public archived threads in a channel (Sorted by archive_timestamp
 * in descending order)
 ...
 * sending another message (0-21600); bots, as well as users with the permission
 ...
 * an event handler.
 */
thread thread_create_sync(const std::string &thread_name, snowflake channel_id,
 ...
 * kilobytes \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
 ...
 * user_identified to user with `dynamic_cast`. \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
 ...
 * - You must have the `MUTE_MEMBERS` permission to unsuppress yourself. You can
 ...
 * should not
 * @param request_to_speak_timestamp The time at which we requested to speak, or
 * 0 to clear the request. The time set here must be the current time or in the
 * future.
 * @throw std::logic_exception You attempted to set a request_to_speak_timestamp
 * in the past which is not the value of 0. \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
 */
confirmation
current_user_set_voice_state_sync(snowflake guild_id, snowflake channel_id,
                                  bool suppress = false,
 ...
 * @return confirmation returned object on completion
 * @param suppress True if the user's audio should be suppressed, false if it
 * should not \memberof dpp::cluster
 * @throw dpp::rest_exception upon failure to execute REST function
 * @warning This function is a blocking (synchronous) call and should only be
 * used from within a separate thread. Avoid direct use of this function inside
 * an event handler.
 */
confirmation user_set_voice_state_sync(snowflake user_id, snowflake guild_id,
                                       snowflake channel_id,
                                       bool suppress = false);
 ...
 * @note unless you want something special from `dpp::user_identified` or you've
 * turned off caching, you have no need to call this. Call `dpp::find_user`
 ...
 *
 * Voice regions per guild are somewhat deprecated in preference of per-channel
 * voice regions. Returns a list of voice region objects for the guild. Unlike
 * the similar /voice route, this returns VIP servers when the guild is
 * VIP-enabled.
 *
 ...
 * be parsed with default allowances, that is, without regard to whether or not
 * an allowed_mentions was present in the request that originally created the
 * message.
 *
 ...
 * an event handler.
 */
message execute_webhook_sync(const class webhook &wh, const struct message &m,
 ...
 * an event handler.
 */
webhook get_webhook_with_token_sync(snowflake webhook_id,
                                    const std::string &token);
