 ...
 * The user can be used to obtain in-depth user details such as if they are
 * nitro, and the guild member information to check their roles on a guild etc.
...
using command_parameter =
	std::variant<std::monostate, std::string, dpp::role, dpp::channel,
                 dpp::resolved_user, int64_t, bool, double>;

/**
 * @brief Parameter types when registering a command.
 * We don't pass these in when triggering the command in the handler, because it
 * is expected the developer added the command so they know what types to expect
 ...
 * Note that for non-slash commands optional parameters can only be at the end
...
struct DPP_EXPORT param_info {
   ...
   * For non-slash commands optional parameters may only be on the end of the
   * list.
   */
  bool optional;
   ...
   */
  param_info(parameter_type t, bool o, const std::string &description,
 ...
 */
using parameter_registration_t =
	std::vector<std::pair<std::string, param_info>>;

/**
 * @brief Parameter list for a called command.
 * See dpp::parameter_registration_t for an explanation as to why vector is
 * used.
 */
 ...
 * to the origin, which may be a slash command or a message. Both require
 * different response facilities but we want this to be transparent if you use
 * the command handler class.
 * @deprecated commandhandler and message commands are deprecated and
 * dpp::slashcommand is encouraged as a replacement.
 */
struct DPP_EXPORT command_source {
  ...
  /**
   * @brief Copy of the underlying message_create_t event, if it was a message
   * create event
   */
  std::optional<message_create_t> message_event;

  /**
   * @brief Copy of the underlying interaction_create_t event, if it was an
...
/**
 * @brief The function definition for a command handler. Expects a command name
 * string, and a list of command parameters.
 * @deprecated commandhandler and message commands are deprecated and
 * dpp::slashcommand is encouraged as a replacement.
 */
using command_handler = std::function<void(
	const std::string &, const parameter_list_t &, command_source)>;

/**
 * @brief Represents the details of a command added to the command handler
 * class.
 * @deprecated commandhandler and message commands are deprecated and
 * dpp::slashcommand is encouraged as a replacement.
 */
struct DPP_EXPORT command_info_t {
  /**
   * @brief Function reference for the handler. This is std::function so it can
   * represent a class member, a lambda or a raw C function pointer.
   */
...
/**
 * @brief The commandhandler class represents a group of commands, prefixed or
 * slash commands with handling functions.
 *
 * It can automatically register slash commands, and handle routing of messages
 * and interactions to separated command handler functions.
 * @deprecated commandhandler and message commands are deprecated and
 * dpp::slashcommand is encouraged as a replacement.
 */
...
private:
   ...
   */
  std::map<dpp::snowflake, std::vector<dpp::slashcommand>>
	  bulk_registration_list_guild;
  /**
...
public:
   ...
   * @param auto_hook_events Set to true to automatically hook the
   * on_interaction_create and on_message events. You should not need to set
   * this to false unless you have a specific use case, as D++ supports multiple
   * listeners to an event, so will allow the commandhandler to hook to your
   * command events without disrupting other uses for the events you may have.
   * @param application_id The application id of the bot. If not specified, the
   * class will look within the cluster object and use cluster::me::id instead.
   */
  commandhandler(class cluster *o, bool auto_hook_events = true,
   ...
   * a global command using the API and you will receive notification of this
   * command via an interaction event.
   ...
   * @param guild_id The guild ID to restrict the command to. For slash commands
   * causes registration of a guild command as opposed to a global command.
   * @return commandhandler& reference to self
   * @throw dpp::logic_exception if application ID cannot be determined
   */
  commandhandler &add_command(const std::string &command,
   ...
   * This method must be called at least once  if you are using the "/" prefix
   * to mark the end of commands being added to the handler. Note that this uses
   * bulk registration and will replace any existing slash commands.
   *
   * Note that if you have previously registered your commands and they have not
   * changed, you do not need to call this again. Discord retains a cache of
   * previously added commands.
   *
   * @note Registration of global slash commands can take up to an hour to
   * appear on Discord. This is a Discord API limitation. For rapid testing use
   * guild specific commands by specifying a guild ID when declaring the
   * command.
   *
   ...
   * dpp::interaction_create_t object if you have disabled automatic
