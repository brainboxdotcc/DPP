...
{
  /**
   * @brief Sending/receiving HTTP headers, acting as a standard HTTP
   * connection. This is the state prior to receiving "HTTP/1.1 101 Switching
   * Protocols" from the server side.
   */
  HTTP_HEADERS,

  /**
   * @brief Connected as a websocket, and "upgraded". Now talking using binary
   * frames.
   */
  CONNECTED
};
...
class DPP_EXPORT websocket_client : public ssl_client
{
   ...
   * @param buffer The buffer to operate on. Will modify the string removing
   * completed items from the head of the queue
   */
  bool parseheader(std::string &buffer);

  /** Unpack a frame and pass completed frames up the stack.
   * @param buffer The buffer to operate on. Gets modified to remove completed
   * frames on the head of the buffer
...
public:
   ...
   */
  websocket_client(const std::string &hostname, const std::string &port = "443",
  ...
  /**
   * @brief Write to websocket. Encapsulates data in frames if the status is
   * CONNECTED.
   *
   ...
   * @return True if the frame was successfully handled. False if no valid frame
   * is in the buffer.
   */
