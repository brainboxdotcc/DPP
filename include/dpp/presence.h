...
/**
 * @brief An activity button is a custom button shown in the rich presence. Can
 * be to join a game or whatever
 */
...
public:
   ...
   */
  std::string label;
  /** The url opened when clicking the button (1-512 characters). It's may be
   * empty
   *
...
/**
 * @brief An activity asset are the images and the hover text displayed in the
 * rich presence
 */
struct DPP_EXPORT activity_assets {
public:
  /** The large asset image which usually contain snowflake ID or prefixed image
   * ID
   */
  ...
  /** The small asset image which usually contain snowflake ID or prefixed image
   * ID
   */
...
/**
 * @brief An activity is a representation of what a user is doing. It might be a
 * game, or a website, or a movie. Whatever.
 */
...
public:
  ...
  /**
   * @brief Get the assets large image url if they have one, otherwise returns
   * an empty string. In case of prefixed image IDs (mp:{image_id}) it returns
   * an empty string.
   *
   * @param size The size of the image in pixels. It can be any power of two
   * between 16 and 4096. if not specified, the default sized image is returned.
   * @return image url or empty string
   */
  [[nodiscard]] std::string get_large_asset_url(uint16_t size = 0) const;

  /**
   * @brief Get the assets small image url if they have one, otherwise returns
   * an empty string. In case of prefixed image IDs (mp:{image_id}) it returns
   * an empty string.
   *
   * @param size The size of the image in pixels. It can be any power of two
   ...
   * @return image url or empty string
   */
  [[nodiscard]] std::string get_small_asset_url(uint16_t size = 0) const;
   ...
   * @param url_ url of the activity, only works for certain sites, such as
   * YouTube
   */
  activity(const activity_type typ, const std::string &nam,
           const std::string &stat, const std::string &url_);
};

/**
 * @brief Represents user presence, e.g. what game they are playing and if they
 * are online
 */
...
public:
  ...
  /** Guild ID. Apparently, Discord supports this internally but the client
   * doesn't... */
  snowflake guild_id;
  ...
  /**
   * @brief Construct a new presence object with some parameters for sending to
   * a websocket
   *
   ...
   */
  presence(presence_status status, activity_type type,
   ...
   *
   * Note: This excludes any part of the presence object that are not valid for
   * websockets and bots, and includes websocket opcode 3. You will not get what
   * you expect if you call this on a user's presence received from
   * on_presence_update or on_guild_create!
   *
