...
struct DPP_EXPORT audit_extra {
  std::string delete_member_days; //!< number of days after which inactive
                                  //!< members were kicked
  ...
  std::string role_name; //!< name of the role if type is "0" (not present if
                         //!< type is "1")
};
...
struct DPP_EXPORT audit_entry {
  ...
  snowflake target_id; //!< id of the affected entity (webhook, user, role,
                       //!< etc.) (may be empty)
  std::vector<audit_change>
	  changes;       //!< Optional: changes made to the target_id
  snowflake user_id; //!< the user who made the changes (may be empty)
  audit_type event;  //!< type of action that occurred
  std::optional<audit_extra>
	  options; //!< Optional: additional info for certain action types
  std::string
	  reason; //!< Optional: the reason for the change (0-512 characters)
};
