 ...
 * Each discord_voice_client connects to one voice channel and derives from a
 * websocket client.
 */
class DPP_EXPORT discord_voice_client : public websocket_client
{
   ...
   * @param to_front If set to true, will place the message at the front of the
   * queue not the back (this is for urgent messages such as heartbeat,
   * presence, so they can take precedence over chunk requests etc)
   */
   ...
   * Multiple opus frames (2880 bytes each) will be encoded into one packet for
   * sending.
   *
   ...
   * @param outDataSize Output data length, should be at least equal to the
   * input size. Will be adjusted on return to the actual compressed data size.
   * @return size_t The compressed data size that was encoded.
   * @throw dpp::voice_exception If data length to encode is invalid or voice
   * support not compiled into D++
   */
  size_t encode(uint8_t *input, size_t inDataSize, uint8_t *output,
                size_t &outDataSize);
...
public:
  ...
  /* This needs to be static, we only initialise libsodium once per program
   * start, so initialising it on first use in a voice connection is best.
   */
   ...
   *
   * @return dpp::utility::uptime Detail of how long the voice client has been
   * connected for
   */
  dpp::utility::uptime get_uptime();

  /** Constructor takes shard id, max shards and token.
   * @param _cluster The cluster which owns this voice connection, for related
   * logging, REST requests etc
   * @param _channel_id The channel id to identify the voice connection as
   * @param _server_id The server id (guild id) to identify the voice connection
   * as
   * @param _token The voice session token to use for identifying to the
   * websocket
   ...
   * @throw dpp::voice_exception Sodium or Opus failed to initialise, or D++ is
   * not compiled with voice support
   */
  discord_voice_client(dpp::cluster *_cluster, snowflake _channel_id,
                       snowflake _server_id, const std::string &_token,
   ...
   * @throw dpp::exception If there was an error processing the frame, or
   * connection to UDP socket failed
   */
   ...
   *
   * @throw dpp::voice_exception If data length is invalid or voice support not
   * compiled into D++
   */
  ...
  discord_voice_client &send_audio_opus(uint8_t *opus_packet,
                                        const size_t length, uint64_t duration);
