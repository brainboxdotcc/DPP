...
int main() {
	...
	bot.on_ready([&bot](const dpp::ready_t & event) {
		if (dpp::run_once<struct register_bot_commands>()) {
			...
			bot.global_command_create(dpp::slashcommand("blep", "Send a random adorable animal photo", bot.me.id)
				.add_option(
					/* If you set the autocomplete setting on a command option, it will trigger the on_autocomplete
					 * event whenever Discord needs to fill information for the choices. You cannot set any choices
					 * here if you set the autocomplete value to true.
					 */
	...
	/* The on_autocomplete event is fired whenever Discord needs information to fill in a command options' choices.
	 * You must reply with a REST event within 500 ms, so make it snappy!
	 */
	bot.on_autocomplete([&bot](const dpp::autocomplete_t & event) {
		for (auto & opt : event.options) {
			...
			if (opt.focused) {
				 ...
				 * Here we can safely know it is a string.
				 */
