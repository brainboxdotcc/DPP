...
int main(int argc, char const *argv[]) {
	 ...
	 * The bot expects opus packets to be 2 channel stereo, 48000 Hz.
	 *
	...
	bot.on_slashcommand([&bot](const dpp::slashcommand_t& event) {
		...
		if (event.command.get_command_name() == "join") {
			...
			/* Now loop through all the pages and send the packets to the vc */
...
You can use `liboggz` to stream an Ogg Opus file to Discord voice channel. `liboggz` provides higher level abstraction and useful APIs. Some features `liboggz` provides include: seeking and timestamp interpretation. Read more on the [documentation](https://www.xiph.org/oggz/doc/).
