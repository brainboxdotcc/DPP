...
uint64_t snowflake_not_null(const json *j, const char *keyname) {
   ...
   * Because discord drinks the javascript kool-aid, they have to send 64 bit
   * integers as strings as js can't deal with them even though we can. So, all
   * snowflakes are sent and received wrapped as string values and must be read
   * by nlohmann::json as string types, then converted from string to uint64_t.
   * Checks for existence of the value, and that it is a string containing a
   * number. If not, then this function returns 0.
   */
  auto k = j->find(keyname);
  if (k != j->end()) {
	return !k->is_null() && k->is_string()
	         ? strtoull(k->get<std::string>().c_str(), nullptr, 10)
	         : 0;
  } else {
	return 0;
  }
...
std::string string_not_null(const json *j, const char *keyname) {
  /* Returns empty string if the value is not a string, or is null or not
   * defined */
  auto k = j->find(keyname);
  if (k != j->end()) {
...
time_t ts_not_null(const json *j, const char *keyname) {
  /* Parses discord ISO 8061 timestamps to time_t, accounting for local time
   * adjustment. Note that discord timestamps contain a decimal seconds part,
   * which time_t and struct tm can't handle. We strip these out.
   */
  time_t retval = 0;
  if (j->find(keyname) != j->end() && !(*j)[keyname].is_null() &&
      (*j)[keyname].is_string()) {
	tm timestamp = {};
	std::string timedate = (*j)[keyname].get<std::string>();
	if (timedate.find('+') != std::string::npos) {
	  std::string tzpart =
		  timedate.substr(timedate.find('+'), timedate.length());
	  if (timedate.find('.') != std::string::npos) {
		timedate = timedate.substr(0, timedate.find('.')); // + "Z" + tzpart;
	  }
	  crossplatform_strptime(timedate.substr(0, 19).c_str(), "%Y-%m-%dT%T",
	                         &timestamp);
	  timestamp.tm_isdst = 0;
	  retval = mktime(&timestamp);
	} else {
	  crossplatform_strptime(timedate.substr(0, 19).c_str(), "%Y-%m-%d %T",
	                         &timestamp);
	  retval = mktime(&timestamp);
	}
  }
...
const std::map<std::string, dpp::events::event *> eventmap = {
	...
	{"MESSAGE_REACTION_REMOVE_ALL",
     new dpp::events::message_reaction_remove_all()                                         },
	{"MESSAGE_REACTION_REMOVE_EMOJI",
     new dpp::events::message_reaction_remove_emoji()                                       },
	...
	{"GUILD_SCHEDULED_EVENT_CREATE",
     new dpp::events::guild_scheduled_event_create()                                        },
	{"GUILD_SCHEDULED_EVENT_UPDATE",
     new dpp::events::guild_scheduled_event_update()                                        },
	{"GUILD_SCHEDULED_EVENT_DELETE",
     new dpp::events::guild_scheduled_event_delete()                                        },
	{"GUILD_SCHEDULED_EVENT_USER_ADD",
     new dpp::events::guild_scheduled_event_user_add()                                      },
	{"GUILD_SCHEDULED_EVENT_USER_REMOVE",
     new dpp::events::guild_scheduled_event_user_remove()                                   },
};

void discord_client::handle_event(const std::string &event, json &j,
                                  const std::string &raw) {
  auto ev_iter = eventmap.find(event);
  if (ev_iter != eventmap.end()) {
	/* A handler with nullptr is silently ignored. We don't plan to make a
	 * handler for it so this usually some user-only thing that's crept into the
	 * API and shown to bots that we dont care about.
	 */
