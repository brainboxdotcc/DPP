...
param_info::param_info(parameter_type t, bool o, const string &d,
                       const std::map<command_value, std::string> &opts)
	: type(t), optional(o), description(d), choices(opts) {}

command_source::command_source(const struct message_create_t &event)
	: guild_id(event.msg.guild_id), channel_id(event.msg.channel_id),
	  command_id(0), issuer(event.msg.author), message_event(event),
	  interaction_event(std::nullopt) {}

command_source::command_source(const struct interaction_create_t &event)
	: guild_id(event.command.guild_id), channel_id(event.command.channel_id),
	  command_id(event.command.id), command_token(event.command.token),
	  issuer(event.command.usr), message_event(std::nullopt),
	  interaction_event(event) {}

commandhandler::commandhandler(cluster *o, bool auto_hook_events,
                               snowflake application_id)
	: slash_commands_enabled(false), owner(o), app_id(application_id) {
  if (!application_id && o->me.id) {
	app_id = o->me.id;
  }
  if (auto_hook_events) {
	interactions = o->on_interaction_create(
		[this](const dpp::interaction_create_t &event) { this->route(event); });
	messages = o->on_message_create(
		[this](const dpp::message_create_t &event) { this->route(event); });
  } else {
...
commandhandler &commandhandler::add_command(
	const std::string &command, const parameter_registration_t &parameters,
	command_handler handler, const std::string &description,
	snowflake guild_id) {
  command_info_t i;
  i.func = handler;
  i.guild_id = guild_id;
  ...
  if (slash_commands_enabled) {
	if (this->app_id == 0) {
	  if (owner->me.id == 0) {
		throw dpp::logic_exception(
			"Command handler not ready (i don't know my application ID)");
	  } else {
	...
	newcommand.set_name(lowercase(command))
		.set_description(description)
		.set_application_id(this->app_id);
	...
	for (auto &parameter : parameters) {
	  ...
	  command_option opt(cot, parameter.first, parameter.second.description,
	                     !parameter.second.optional);
	...
	if (guild_id) {
	  if (bulk_registration_list_guild.find(guild_id) ==
	      bulk_registration_list_guild.end()) {
...
commandhandler &commandhandler::register_commands() {
  for (auto &guild_commands : bulk_registration_list_guild) {
	owner->guild_bulk_command_create(
		guild_commands.second, guild_commands.first,
		[guild_commands, this](const dpp::confirmation_callback_t &callback) {
		  if (callback.is_error()) {
			this->owner->log(dpp::ll_error,
		                     fmt::format("Failed to register guild slash "
		                                 "commands for guild id '{}': {}",
		                                 guild_commands.first,
		                                 callback.http_info.body));
		  }
		});
  }
  owner->global_bulk_command_create(
	  bulk_registration_list_global,
...
/* Note that message based command routing relies on cache to resolve ping types
 * (e.g. user, channel ping). There isn't really a way around this for many
 * things because there is no 'resolved' member for it. We only get resolved
 * information for the user issuing the command.
 */
	  ...
	  for (auto &p : found_cmd->second.parameters) {
		...
		case pt_role: {
		  ...
		  if (x.length() > 4 && x[0] == '<' && x[1] == '&') {
			auto rid = from_string<uint64_t>(x.substr(2, x.length() - 1));
		...
		case pt_channel: {
		  ...
		  ss >> x;
		  if (x.length() > 4 && x[0] == '<' && x[1] == '#') {
			auto cid = from_string<uint64_t>(x.substr(2, x.length() - 1));
		...
		case pt_user: {
		  ...
		  ss >> x;
		  if (x.length() > 4 && x[0] == '<' && x[1] == '@') {
			auto uid = from_string<uint64_t>(x.substr(2, x.length() - 1));
...
void commandhandler::route(const struct interaction_create_t &event) {
  ...
  if (found_cmd != commands.end()) {
	...
	for (auto &p : found_cmd->second.parameters) {
	  ...
	  if (p.second.optional &&
	      slash_parameter.index() == 0 /* std::monostate */) {
	  ...
	  case pt_user: {
		...
		if (u) {
		  ...
		  /* Otherwise use interaction resolved fields */
		  if (event.command.resolved.users.find(uid) !=
		          event.command.resolved.users.end() &&
		      event.command.resolved.members.find(uid) !=
		          event.command.resolved.members.end()) {
...
void commandhandler::reply(const dpp::message &m, command_source source,
  ...
  if (!source.command_token.empty() && source.command_id) {
	owner->interaction_response_create(
		source.command_id, source.command_token,
		dpp::interaction_response(ir_channel_message_with_source, msg),
		callback);
  } else {
...
void commandhandler::thinking(command_source source,
  ...
  if (!source.command_token.empty() && source.command_id) {
	owner->interaction_response_create(
		source.command_id, source.command_token,
		dpp::interaction_response(ir_deferred_channel_message_with_source, msg),
		callback);
  }
}
