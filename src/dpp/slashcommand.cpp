...
command_option& command_option::set_min_length(uint16_t min_v) {
	min_length = min_v;
	return *this;
}

command_option& command_option::set_max_length(uint16_t max_v) {
	if (max_v < 1) {
		max_v = 1;
	}
	max_length = max_v;
...
void to_json(json& j, const command_option& opt) {
	...
	if (opt.type == dpp::co_string) {
		j["min_length"] = opt.min_length;
		j["max_length"] = opt.max_length;
...
command_option &command_option::fill_from_json(nlohmann::json *j) {
	...
	*/
	std::function<void(nlohmann::json *, command_option &, uint8_t)> fill = [&fill](nlohmann::json *j, command_option &o, uint8_t depth) {
		...
		o.min_length = int16_not_null(j, "min_length");
		o.max_length = int16_not_null(j, "max_length");
