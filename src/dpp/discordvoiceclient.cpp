...
struct rtp_header {
  ...
  rtp_header(uint16_t _seq, uint32_t _ts, uint32_t _ssrc)
	  : constant(htons(0x8078)), sequence(htons(_seq)), timestamp(htonl(_ts)),
		ssrc(htonl(_ssrc)) {}
};

bool discord_voice_client::sodium_initialised = false;

discord_voice_client::discord_voice_client(dpp::cluster *_cluster,
                                           snowflake _channel_id,
                                           snowflake _server_id,
                                           const std::string &_token,
                                           const std::string &_session_id,
                                           const std::string &_host)
	: websocket_client(_host.substr(0, _host.find(":")),
                       _host.substr(_host.find(":") + 1, _host.length()),
                       "/?v=4", OP_TEXT),
	  runner(nullptr), connect_time(0), port(0), ssrc(0), timescale(1000000),
	  paused(false),
#if HAVE_VOICE
	  encoder(nullptr), decoder(nullptr), repacketizer(nullptr),
#endif
	  fd(INVALID_SOCKET), secret_key(nullptr), sequence(0), timestamp(0),
	  last_timestamp(std::chrono::high_resolution_clock::now()), sending(false),
	  tracks(0), creator(_cluster), terminating(false),
#ifdef HAVE_VOICE
	  decode_voice_recv(true),
#else
	  decode_voice_recv(false),
#endif
	  heartbeat_interval(0), last_heartbeat(time(nullptr)), token(_token),
	  sessionid(_session_id), server_id(_server_id), channel_id(_channel_id) {
#if HAVE_VOICE
  if (!discord_voice_client::sodium_initialised) {
	if (sodium_init() < 0) {
	  throw dpp::voice_exception(
		  "discord_voice_client::discord_voice_client; sodium_init() failed");
	}
  ...
  if (opusError) {
	throw dpp::voice_exception("discord_voice_client::discord_voice_client; "
  ...
  if (!repacketizer) {
	throw dpp::voice_exception("discord_voice_client::discord_voice_client; "
	                           "opus_repacketizer_create() failed");
  }
  this->connect();
#else
...
int discord_voice_client::udp_send(const char *data, size_t length) {
  ...
  return sendto(this->fd, data, (int)length, 0, (const sockaddr *)&servaddr,
...
bool discord_voice_client::handle_frame(const std::string &data) {
  ...
  try {
	...
	log(dpp::ll_error, std::string("discord_voice_client::handle_frame ") +
	                       e.what() + ": " + data);
	return true;
  }
  ...
  if (j.find("op") != j.end()) {
	...
	/* Client Disconnect */
	case 13: {
	  if (j.find("d") != j.end() && j["d"].find("user_id") != j["d"].end() &&
	      !j["d"]["user_id"].is_null()) {
		snowflake u_id = snowflake_not_null(&j["d"], "user_id");
		auto it =
			std::find_if(ssrcMap.begin(), ssrcMap.end(),
	...
	case 12: {
	  if (j.find("d") != j.end() && j["d"].find("user_id") != j["d"].end() &&
	      !j["d"]["user_id"].is_null() && j["d"].find("ssrc") != j["d"].end() &&
	      !j["d"]["ssrc"].is_null() && j["d"]["ssrc"].is_number_integer()) {
	...
	case 8: {
	  if (j.find("d") != j.end() &&
	      j["d"].find("heartbeat_interval") != j["d"].end() &&
	      !j["d"]["heartbeat_interval"].is_null()) {
	...
	case 4: {
	  ...
	  /* This is needed to start voice receiving and make sure that the start of
	   * sending isn't cut off */
	  send_silence(20);
	...
	case 2: {
	  ...
	  log(ll_debug, fmt::format("Voice websocket established; UDP endpoint: "
	                            "{}:{} [ssrc={}] with {} modes",
	                            ip, port, ssrc, modes.size()));
	  ...
	  if ((newfd = ::socket(AF_INET, SOCK_DGRAM, 0)) >= 0) {
		...
		if (result != NO_ERROR)
		  throw dpp::connection_exception(
			  "Can't switch socket to non-blocking mode!");
#else
		int ofcmode;
		...
		this->fd = newfd;
		this->custom_writeable_fd =
			std::bind(&discord_voice_client::want_write, this);
		this->custom_readable_fd =
			std::bind(&discord_voice_client::want_read, this);
		this->custom_writeable_ready =
			std::bind(&discord_voice_client::write_ready, this);
		this->custom_readable_ready =
			std::bind(&discord_voice_client::read_ready, this);
...
void discord_voice_client::pause_audio(bool pause) { this->paused = pause; }
...
void discord_voice_client::write_ready() {
  ...
  uint64_t bufsize = 0;
  {
	...
	if (!this->paused && outbuf.size()) {
	  ...
	  if (outbuf[0].packet.size() == 2 &&
	      ((uint16_t)(*(outbuf[0].packet.data()))) == AUDIO_TRACK_MARKER) {
	  ...
	  if (outbuf.size()) {
		if (this->udp_send(outbuf[0].packet.data(),
		                   outbuf[0].packet.length()) ==
		    (int)outbuf[0].packet.length()) {
  ...
  if (duration) {
	std::chrono::nanoseconds latency =
		std::chrono::duration_cast<std::chrono::nanoseconds>(
			std::chrono::high_resolution_clock::now() - last_timestamp);
	std::chrono::nanoseconds sleep_time =
		std::chrono::nanoseconds(duration) - latency;
...
void discord_voice_client::error(uint32_t errorcode) {
  ...
  log(dpp::ll_warning, fmt::format("Voice session error: {} on channel {}: {}",
                                   errorcode, channel_id, error));

  /* Errors 4004...4016 except 4014 are fatal and cause termination of the voice
   * session */
  if (errorcode >= 4003) {
	stop_audio();
	this->terminating = true;
	log(dpp::ll_error,
	    "This is a non-recoverable error, giving up on voice connection");
  }
}

void discord_voice_client::log(dpp::loglevel severity,
...
void discord_voice_client::one_second_timer() {
  ...
  if (this->get_state() == CONNECTED) {
	...
	if (this->heartbeat_interval) {
	  ...
	  if (time(nullptr) >
	      last_heartbeat + ((heartbeat_interval / 1000.0) * 0.75)) {
		queue_message(json({
							   {"op", 3     },
							   {"d",  rand()}
        })
...
size_t discord_voice_client::encode(uint8_t *input, size_t inDataSize,
                                    uint8_t *output, size_t &outDataSize) {
#if HAVE_VOICE
  outDataSize = 0;
  ...
  if (0 == (inDataSize % mEncFrameBytes)) {
	...
	for (size_t i = 0; i < (inDataSize / mEncFrameBytes); ++i) {
	  ...
	  if (ret > 0) {
		...
		if (retval != OPUS_OK) {
		  isOk = false;
		  log(ll_warning, fmt::format("opus_repacketizer_cat(): {}",
		                              opus_strerror(retval)));
		  break;
		}
		out += ret;
	  } else {
	...
	if (isOk) {
	  ...
	  if (ret > 0) {
		...
		log(ll_warning,
		    fmt::format("opus_repacketizer_out(): {}", opus_strerror(ret)));
	  }
	}
  } else {
	throw dpp::voice_exception(
		fmt::format("Invalid input data length: {}, must be n times of {}",
...
void discord_voice_client::skip_to_next_marker() {
  std::lock_guard<std::mutex> lock(this->stream_mutex);
  /* Keep popping the first entry off the outbuf until the first entry is a
   * track marker */
  while (!outbuf.empty() && outbuf[0].packet.size() != sizeof(uint16_t) &&
         ((uint16_t)(*(outbuf[0].packet.data()))) != AUDIO_TRACK_MARKER) {
	outbuf.erase(outbuf.begin());
  }
  if (outbuf.size()) {
...
discord_voice_client &
discord_voice_client::send_audio_raw(uint16_t *audio_data,
                                     const size_t length) {
#if HAVE_VOICE
  ...
  if (length > max_frame_bytes) {
	...
	while (s_audio_data.length() > max_frame_bytes) {
	  ...
	  s_audio_data.erase(s_audio_data.begin(),
	                     s_audio_data.begin() + max_frame_bytes);
  ...
  encodedAudioLength =
	  this->encode((uint8_t *)audio_data, length, encodedAudioData.data(),
                   encodedAudioLength);
...
discord_voice_client &
discord_voice_client::send_audio_opus(uint8_t *opus_packet,
                                      const size_t length) {
#if HAVE_VOICE
  int samples =
	  opus_packet_get_nb_samples(opus_packet, (opus_int32)length, 48000);
...
discord_voice_client::send_audio_opus(uint8_t *opus_packet, const size_t length,
  ...
  std::vector<uint8_t> audioDataPacket(sizeof(header) + encodedAudioLength +
                                       crypto_secretbox_MACBYTES);
  std::memcpy(audioDataPacket.data(), &header, sizeof(header));

  crypto_secretbox_easy(audioDataPacket.data() + sizeof(header),
                        encodedAudioData.data(), encodedAudioLength,
                        (const unsigned char *)nonce, secret_key);

  this->send((const char *)audioDataPacket.data(), audioDataPacket.size(),
             duration);
  timestamp += frameSize;
...
std::string discord_voice_client::discover_ip() {
  ...
  if ((newfd = ::socket(AF_INET, SOCK_DGRAM, 0)) >= 0) {
	...
	servaddr.sin_port = htons(this->port);
	servaddr.sin_addr.s_addr = inet_addr(this->ip.c_str());
	if (::connect(newfd, (const sockaddr *)&servaddr, sizeof(sockaddr_in)) <
	    0) {
	...
	return string{(const char *)(packet + 8)};
  }
  return "";
}
