...
channel_stream::channel_stream(cluster& bot, channel out_channel_id) {
	this->bot = bot;
...
dm_stream::dm_stream(cluster& bot, user out_dm) {
	this->bot = bot;
	this->out_user_id = out_user.id;
}
void dm_stream::send(command_completion_event_t callback) {
	
	this->bot.message_create(this->out_user_id,this->msg,callback);
}
