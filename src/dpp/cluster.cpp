 ...
 * methods like cluster::audit_reason_get and cluster::audit_reason_set thread safe across
...
cluster& cluster::websocket_protocol_set(websocket_protocol_t mode) {
	ws_mode = mode;
	return *this;
}
...
snowflake cluster::dm_channel_get(snowflake user_id) {
...
void cluster::dm_channel_set(snowflake user_id, snowflake channel_id) {
	std::lock_guard<std::mutex> lock(dm_list_lock);
...
void cluster::post_rest(const std::string &endpoint, const std::string &major_parameters, const std::string &parameters, http_method method, const std::string &postdata, json_encode_t callback, const std::string &filename, const std::string &filecontent) {
	...
	}, postdata, method, audit_reason_get(), filename, filecontent));
}
...
cluster& cluster::audit_reason_set(const std::string &reason) {
	audit_reason = reason;
	return *this;
}

cluster& cluster::audit_reason_clear() {
	audit_reason.clear();
	return *this;
}

std::string cluster::audit_reason_get() {
	std::string r = audit_reason;
	audit_reason.clear();
	return r;
}

discord_client* cluster::shard_get(uint32_t id) {
...
const shard_list& cluster::shards_get() {
	return shards;
}
