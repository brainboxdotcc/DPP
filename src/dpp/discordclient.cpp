...
#define PATH_COMPRESSED_JSON                                                   \
  "/?v=" DISCORD_API_VERSION "&encoding=json&compress=zlib-stream"
#define PATH_UNCOMPRESSED_ETF "/?v=" DISCORD_API_VERSION "&encoding=etf"
#define PATH_COMPRESSED_ETF                                                    \
  "/?v=" DISCORD_API_VERSION "&encoding=etf&compress=zlib-stream"
...
discord_client::discord_client(dpp::cluster *_cluster, uint32_t _shard_id,
                               uint32_t _max_shards, const std::string &_token,
                               uint32_t _intents, bool comp,
                               websocket_protocol_t ws_proto)
	: websocket_client(DEFAULT_GATEWAY, "443",
                       comp ? (ws_proto == ws_json ? PATH_COMPRESSED_JSON
                                                   : PATH_COMPRESSED_ETF)
                            : (ws_proto == ws_json ? PATH_UNCOMPRESSED_JSON
                                                   : PATH_UNCOMPRESSED_ETF)),
	  terminating(false), runner(nullptr), compressed(comp),
	  decomp_buffer(nullptr), decompressed_total(0), connect_time(0),
	  ping_start(0.0), creator(_cluster), heartbeat_interval(0),
	  last_heartbeat(time(nullptr)), shard_id(_shard_id),
	  max_shards(_max_shards), last_seq(0), token(_token), intents(_intents),
	  sessionid(""), resumes(0), reconnects(0), websocket_ping(0.0),
	  ready(false), last_heartbeat_ack(time(nullptr)), protocol(ws_proto) {
...
void discord_client::setup_zlib() {
  if (compressed) {
	zlib->d_stream.zalloc = (alloc_func) nullptr;
	zlib->d_stream.zfree = (free_func) nullptr;
	zlib->d_stream.opaque = (voidpf) nullptr;
  ...
  do {
	...
	do {
	  ...
	  try {
		...
		log(dpp::ll_error,
		    std::string("Error establishing connection, retry in 5 seconds: ") +
		        e.what());
		ssl_client::close();
...
bool discord_client::handle_frame(const std::string &buffer) {
  ...
  if (compressed) {
	...
	if ((uint8_t)buffer[buffer.size() - 4] == 0x00 &&
	    (uint8_t)buffer[buffer.size() - 3] == 0x00 &&
	    (uint8_t)buffer[buffer.size() - 2] == 0xFF &&
  ...
  /**
   * This section parses the input frames from the websocket after they're
   * decompressed. Note that both ETF and JSON parsers return an nlohmann::json
   * object, so that the rest of the library or any user code does not need to
   * be concerned with protocol differences. Generally, ETF is faster and
   * consumes much less memory, but provides less opportunities to diagnose if
   * it goes wrong.
   */
  switch (protocol) {
  case ws_json:
	try {
	  ...
	  log(dpp::ll_error,
	      fmt::format("discord_client::handle_frame(JSON): {} [{}]", e.what(),
	                  data));
	  return true;
	}
	break;
  case ws_etf:
	try {
	  ...
	  log(dpp::ll_error,
	      fmt::format(
			  "discord_client::handle_frame(ETF): {} len={}\n{}", e.what(),
			  data.size(),
  ...
  if (o != j.end() && !o->is_null()) {
	...
	case 9:
	  ...
	  op = 10;
	  log(dpp::ll_debug,
	      fmt::format("Failed to resume session {}, will reidentify",
	                  sessionid));
	  this->sessionid = "";
	  this->last_seq = 0;
	  ...
	  [[fallthrough]];
	case 10:
	  /* Need to check carefully for the existence of this before we try to
	   * access it! */
	  if (j.find("d") != j.end() &&
	  ...
	  if (last_seq && !sessionid.empty()) {
		/* Resume */
		log(dpp::ll_debug, fmt::format("Resuming session {} with seq={}",
		                               sessionid, last_seq));
		json obj = {
			{"op", 6                  },
			{"d",
		...
		while (time(nullptr) < creator->last_identify + 5) {
		  ...
		  log(dpp::ll_debug,
		      fmt::format(
				  "Waiting {} seconds before identifying for session...",
				  wait));
	...
	case 7:
	  log(dpp::ll_debug,
	      fmt::format("Reconnection requested, closing socket {}", sessionid));
	  message_queue.clear();
...
void discord_client::error(uint32_t errorcode) {
  ...
  log(dpp::ll_warning,
      fmt::format("OOF! Error from underlying websocket: {}: {}", errorcode,
                  error));
}
...
void discord_client::one_second_timer() {
  ...
  /* This all only triggers if we are connected (have completed websocket, and
   * received READY or RESUMED) */
  if (this->is_connected()) {

	/* If we stopped getting heartbeat acknowledgements, this means the
	 * connections is dead. This can happen to TCP connections which is why we
	 * have heartbeats in the first place. Miss two ACKS, forces a reconnection.
	 */
	if ((time(nullptr) - this->last_heartbeat_ack) > heartbeat_interval * 2) {
	  log(dpp::ll_warning,
	      fmt::format(
			  "Missed heartbeat ACK, forcing reconnection to session {}",
			  sessionid));
	  message_queue.clear();
	...
	/* Send pings (heartbeat opcodes) before each interval. We send them
	 * slightly more regular than expected, just to be safe.
	 */
...
void discord_client::connect_voice(snowflake guild_id, snowflake channel_id,
  ...
  if (connecting_voice_channels.find(guild_id) ==
      connecting_voice_channels.end()) {
	 ...
	 */
	log(ll_debug, fmt::format("Sending op 4 to join VC, guild {} channel {} ",
	                          guild_id, channel_id));
	queue_message(jsonobj_to_string(json({
					  {"op", 4},
					  {"d",
	...
	log(ll_debug,
	    fmt::format("Requested the bot connect to voice channel {} on guild "
	                "{}, but it seems we are already on this VC",
	                channel_id, guild_id));
  }
#endif
}
...
void discord_client::disconnect_voice_internal(snowflake guild_id,
                                               bool emit_json) {
#ifdef HAVE_VOICE
  std::lock_guard<std::mutex> lock(voice_mutex);
...
voiceconn::voiceconn(discord_client *o, snowflake _channel_id)
	: creator(o), channel_id(_channel_id), voiceclient(nullptr) {}
...
void voiceconn::connect(snowflake guild_id) {
  if (this->is_ready() && !this->is_active()) {
	/* This is wrapped in a thread because instantiating discord_voice_client
	 * can initiate a blocking SSL_connect() */
	auto t = std::thread([guild_id, this]() {
	  try {
		this->creator->log(
			ll_debug, fmt::format("Connecting voice for guild {} channel {}",
		                          guild_id, this->channel_id));
		this->voiceclient = new discord_voice_client(
			creator->creator, this->channel_id, guild_id, this->token,
			this->session_id, this->websocket_hostname);
		/* Note: Spawns thread! */
		this->voiceclient->run();
	  } catch (std::exception &e) {
		this->creator->log(ll_error,
		                   fmt::format("Can't connect to voice websocket "
		                               "(guild_id: {}, channel_id: {}): {}",
		                               guild_id, this->channel_id, e.what()));
	  }
	});
	t.detach();
  }
