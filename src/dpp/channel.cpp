...
channel::channel()
	: managed(), owner_id(0), parent_id(0), guild_id(0), last_message_id(0),
	  last_pin_timestamp(0), permissions(0), flags(0), position(0), bitrate(0),
	  rate_limit_per_user(0), user_limit(0) {}
...
channel &channel::set_name(const std::string &name) {
  this->name =
	  utility::validate(name, 1, 100, "name must be at least 1 character");
  return *this;
}
...
channel &channel::set_nsfw(const bool is_nsfw) {
  this->flags =
	  (is_nsfw) ? this->flags | dpp::c_nsfw : this->flags & ~dpp::c_nsfw;
  ;
  return *this;
}
...
channel &channel::add_permission_overwrite(const snowflake id,
                                           const uint8_t type,
...
bool channel::is_text_channel() const { return flags & dpp::c_text; }

bool channel::is_dm() const { return flags & dpp::c_dm; }

bool channel::is_voice_channel() const { return flags & dpp::c_voice; }

bool channel::is_group_dm() const {
  return (flags & (dpp::c_dm | dpp::c_group)) == (dpp::c_dm | dpp::c_group);
}

bool channel::is_category() const { return flags & dpp::c_category; }
...
bool thread::is_news_thread() const { return flags & dpp::c_news_thread; }

bool thread::is_public_thread() const { return flags & dpp::c_public_thread; }

bool thread::is_private_thread() const { return flags & dpp::c_private_thread; }
...
thread &thread::fill_from_json(json *j) {
  ...
  metadata.auto_archive_duration =
	  int16_not_null(&json_metadata, "auto_archive_duration");
...
channel &channel::fill_from_json(json *j) {
  ...
  if (vqm == 2) {
	/* If this is set to 2, this means full quality 720p video for voice
	 * channel. Undefined, or a value of 1 (the other two possibilities right
	 * now) means video quality AUTO.
	 */
  ...
  /* Note: This is only set when the channel is in the resolved set from an
   * interaction. When set it contains the invokers permissions on channel. Any
   * other time, contains 0.
   */
...
std::string channel::get_banner_url(uint16_t size) const {
  /* XXX: Discord were supposed to change their CDN over to discord.com, they
   * haven't. At some point in the future this URL *will* change!
   */
  if (!this->banner.to_string().empty()) {
	// TODO implement this, endpoint for that isn't finished yet
