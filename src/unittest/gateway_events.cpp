...
void gateway_events_tests(const std::string& token, dpp::cluster& bot) {
	std::vector<std::byte> test_image = load_test_data("DPP-Logo.png");
					...
					if (!callback.is_error()) {
						...
						bot.guild_command_delete(s.id, TEST_GUILD_ID, [&](const dpp::confirmation_callback_t &callback) {
							if (!callback.is_error()) {
								...
								test_message.add_file("test.png", std::string{ reinterpret_cast<const char*>(test_image.data()), test_image.size() }, "image/png");
